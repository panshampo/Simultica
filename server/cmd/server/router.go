...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	if larkKey, err := secretbox.LoadKey("MULTICA_LARK_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				reactionTracker := lark.NewReactionTracker()
				threadAnchors := lark.NewThreadAnchorTracker()
				patcher := lark.NewPatcher(queries, installSvc, larkClient, lark.PatcherConfig{})
				patcher.SetReactionTracker(reactionTracker)
				patcher.SetThreadAnchorTracker(threadAnchors)
				if store != nil {
					patcher.SetAttachmentReader(lark.StorageReaderAdapter{Storage: store})
				}
	...
	r.Route("/api/daemon", func(r chi.Router) {
		...
		r.Post("/tasks/{taskId}/attachments", h.UploadTaskAttachment)
