...
func init() {
	...
	issueCreateCmd.Flags().String("origin-type", "", "Issue origin type for trusted automation")
	issueCreateCmd.Flags().String("origin-id", "", "Issue origin UUID for trusted automation")
...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	if originType != "" || originID != "" {
		if originType == "" || originID == "" {
			return fmt.Errorf("--origin-type and --origin-id must be provided together")
		}
