...
func init() {
	...
	projectResourceAddCmd.Flags().String("type", "github_repo", "Resource type (e.g. github_repo, local_directory, external_directory — see docs)")
	...
	projectResourceAddCmd.Flags().String("local-path", "", "Shortcut: absolute local path (used by local_directory and external_directory)")
	projectResourceAddCmd.Flags().String("daemon-id", "", "Shortcut: id of the daemon that owns the local path (used by local_directory and external_directory)")
	projectResourceAddCmd.Flags().String("ref-label", "", "Shortcut: optional label embedded in resource_ref (used by local_directory and external_directory)")
	...
	projectResourceUpdateCmd.Flags().String("local-path", "", "Shortcut: new absolute local path (local_directory or external_directory)")
	projectResourceUpdateCmd.Flags().String("daemon-id", "", "Shortcut: new daemon id (local_directory or external_directory)")
	projectResourceUpdateCmd.Flags().String("ref-label", "", "Shortcut: new label embedded in resource_ref (local_directory or external_directory)")
...
func runProjectResourceAdd(cmd *cobra.Command, args []string) error {
	...
	if rawRef, _ := cmd.Flags().GetString("ref"); strings.TrimSpace(rawRef) != "" {
		...
		case "github_repo":
			...
			body["resource_ref"] = ref
		case "local_directory", "external_directory":
			...
			if pathVal == "" || daemonVal == "" {
				return fmt.Errorf("%s requires --local-path and --daemon-id (or pass a JSON payload via --ref)", resourceType)
			}
...
func buildResourceRefFromFlags(cmd *cobra.Command, resourceType string, existingRef map[string]any) (map[string]any, bool, error) {
	...
	case "local_directory", "external_directory":
		...
		if v, ok := ref["local_path"].(string); !ok || v == "" {
			return nil, false, fmt.Errorf("%s: --local-path is required (no existing local_path to merge with)", resourceType)
		}
		if v, ok := ref["daemon_id"].(string); !ok || v == "" {
			return nil, false, fmt.Errorf("%s: --daemon-id is required (no existing daemon_id to merge with)", resourceType)
		}
		return ref, true, nil
	default:
...
// resource_ref object — for github_repo this is the URL; for local_directory
// and external_directory it is the local path.
