...
// coding agents (Claude Code, DS Copilot, Codex, Copilot, OpenCode, OpenClaw, Hermes,
// Gemini, Pi, Cursor, Kimi, Kiro, Antigravity, Traex). It mirrors the
...
type Config struct {
	ExecutablePath string            // path to CLI binary (claude, ds-copilot, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor, kimi, kiro-cli, agy, traex)
	ExecutableArgs []string          // command prefix args before backend-managed args (for example: bash ~/.ds-copilot/launch.sh)
	Env            map[string]string // extra environment variables
	Logger         *slog.Logger
}

// New creates a Backend for the given agent type.
// Supported types: "claude", "ds-copilot", "codex", "copilot", "opencode", "openclaw", "hermes", "gemini", "pi", "cursor", "kimi", "kiro", "antigravity", "traex".
...
func DetectVersion(ctx context.Context, executablePath string) (string, error) {
	return DetectVersionWithArgs(ctx, executablePath, nil)
}

// DetectVersionWithArgs runs the agent CLI prefix with --version and returns
// the output. executableArgs are inserted before --version for runtimes that
// launch through an interpreter wrapper, such as `bash launch.sh --version`.
func DetectVersionWithArgs(ctx context.Context, executablePath string, executableArgs []string) (string, error) {
	return detectCLIVersion(ctx, executablePath, executableArgs)
}
