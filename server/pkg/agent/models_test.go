...
func TestDsCopilotModelsAreManagedByRuntime(t *testing.T) {
	t.Parallel()

	got, err := ListModels(context.Background(), "ds-copilot", "/nonexistent/ds-copilot")
	if err != nil {
		t.Fatalf("ListModels(ds-copilot) error: %v", err)
	}
	if len(got) != 0 {
		t.Fatalf("ds-copilot model catalog should be runtime-managed, got %+v", got)
	}
	if ModelSelectionSupported("ds-copilot") {
		t.Fatal("ds-copilot should report model selection unsupported because the launcher owns CCR model selection")
	}
}
