...
type claudeExecutionOptions struct {
	defaultExecutable string
	agentLabel        string
}

func (b *claudeBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	return b.execute(ctx, prompt, opts, claudeExecutionOptions{
		defaultExecutable: "claude",
		agentLabel:        "claude",
	})
}

func (b *claudeBackend) execute(ctx context.Context, prompt string, opts ExecOptions, execOpts claudeExecutionOptions) (*Session, error) {
	agentLabel := execOpts.agentLabel
	if agentLabel == "" {
		agentLabel = "claude"
	}
	defaultExecutable := execOpts.defaultExecutable
	if defaultExecutable == "" {
		defaultExecutable = "claude"
	}
	execPath := b.cfg.ExecutablePath
	if execPath == "" {
		execPath = defaultExecutable
	}
	if _, err := exec.LookPath(execPath); err != nil {
		return nil, fmt.Errorf("%s executable not found at %q: %w", agentLabel, execPath, err)
	}

	timeout := opts.Timeout
	runCtx, cancel := runContext(ctx, timeout)

	args := append([]string(nil), b.cfg.ExecutableArgs...)
	args = append(args, buildClaudeArgs(opts, b.cfg.Logger)...)
	...
	if err != nil {
		cancel()
		return nil, fmt.Errorf("%s stdout pipe: %w", agentLabel, err)
	}
	stdin, err := cmd.StdinPipe()
	if err != nil {
		cancel()
		return nil, fmt.Errorf("%s stdin pipe: %w", agentLabel, err)
	}
	...
	stderrBuf := newStderrTail(newLogWriter(b.cfg.Logger, "["+agentLabel+":stderr] "), agentStderrTailBytes)
	cmd.Stderr = stderrBuf

	if err := cmd.Start(); err != nil {
		closeStdin()
		cancel()
		return nil, fmt.Errorf("start %s: %w", agentLabel, err)
	}

	b.cfg.Logger.Info(agentLabel+" started", "pid", cmd.Process.Pid, "cwd", opts.Cwd, "model", opts.Model)
	...
	go func() {
		...
		case runCtx.Err() == context.DeadlineExceeded:
			finalStatus = "timeout"
			finalError = fmt.Sprintf("%s timed out after %s", agentLabel, timeout)
		case runCtx.Err() == context.Canceled:
			finalStatus = "aborted"
			finalError = "execution cancelled"
		case writeErr != nil && finalStatus == "completed" && sessionID == "":
			// No result event landed and the prompt write failed -- the CLI
			// died before reading the prompt. Surface the write error; the
			// stderr tail attached below carries the real reason.
			finalStatus = "failed"
			finalError = fmt.Sprintf("write %s input: %v", agentLabel, writeErr)
		case exitErr != nil && finalStatus == "completed":
			finalStatus = "failed"
			finalError = fmt.Sprintf("%s exited with error: %v", agentLabel, exitErr)
		}
		...
		if finalError != "" {
			finalError = withAgentStderr(finalError, agentLabel, stderrBuf.Tail())
		}

		b.cfg.Logger.Info(agentLabel+" finished", "pid", cmd.Process.Pid, "status", finalStatus, "duration", duration.Round(time.Millisecond).String())

		reportedSessionID := resolveSessionID(opts.ResumeSessionID, sessionID, finalStatus == "failed")
		if reportedSessionID != sessionID {
			b.cfg.Logger.Info(agentLabel+" resume did not land; clearing fresh session id for daemon fallback",
...
func detectCLIVersion(ctx context.Context, execPath string, execArgs []string) (string, error) {
