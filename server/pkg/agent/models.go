...
func ListModels(ctx context.Context, providerType, executablePath string) ([]Model, error) {
	...
	case "codex":
		...
		return models, nil
	case "traex":
		// Traex is a Codex fork; the `app-server` JSON-RPC contract, the
		// `debug models` catalog, and the reasoning-effort vocabulary are
		// all bit-compatible against traecli 0.200.7. Reuse the codex
		// static catalog + thinking-level annotator so users get the
		// same picker behaviour without a parallel fixture.
		models := codexStaticModels()
		annotateCodexThinking(ctx, models, executablePath)
		return models, nil
	case "gemini":
...
// any effect for the given provider. DS Copilot is deliberately false:
// its launcher selects the CCR model before execing Claude Code, and the
// backend strips opts.Model to avoid a second conflicting --model flag.
//
