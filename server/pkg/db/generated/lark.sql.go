...
RETURNING id, chat_session_id, installation_id, lark_chat_id, lark_chat_type, created_at, lark_thread_root_id, issue_id
`
...
type CreateLarkChatSessionBindingParams struct {
	...
	LarkThreadRootID string      `json:"lark_thread_root_id"`
...
SELECT id, chat_session_id, installation_id, lark_chat_id, lark_chat_type, created_at, lark_thread_root_id, issue_id FROM lark_chat_session_binding
...
func (q *Queries) GetLarkChatSessionBinding(ctx context.Context, arg GetLarkChatSessionBindingParams) (LarkChatSessionBinding, error) {
	row := q.db.QueryRow(ctx, getLarkChatSessionBinding, arg.InstallationID, arg.LarkChatID, arg.LarkThreadRootID)
	var i LarkChatSessionBinding
	err := row.Scan(
		&i.ID,
		&i.ChatSessionID,
...
const setLarkChatSessionBindingIssue = `-- name: SetLarkChatSessionBindingIssue :exec
UPDATE lark_chat_session_binding
SET issue_id = $2
WHERE id = $1 AND issue_id IS NULL
`

type SetLarkChatSessionBindingIssueParams struct {
	ID      pgtype.UUID `json:"id"`
	IssueID pgtype.UUID `json:"issue_id"`
}
...
func (q *Queries) SetLarkChatSessionBindingIssue(ctx context.Context, arg SetLarkChatSessionBindingIssueParams) error {
	_, err := q.db.Exec(ctx, setLarkChatSessionBindingIssue, arg.ID, arg.IssueID)
	return err
}
