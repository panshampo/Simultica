...
INSERT INTO lark_chat_session_binding (
    ...
    lark_thread_root_id, issue_id
) VALUES (
    $1, $2, $3, $4, $5, sqlc.narg('issue_id')
)
RETURNING *;

-- name: GetLarkChatSessionBinding :one
-- Lookup-by-Lark-thread path. Used by the inbound dispatcher to find the
-- existing chat_session before deciding whether to create one. The
-- UNIQUE (installation_id, lark_chat_id, lark_thread_root_id) constraint
-- means at most one row matches per thread; p2p messages always pass an
-- empty thread_root_id.
SELECT * FROM lark_chat_session_binding
WHERE installation_id = $1
  AND lark_chat_id = $2
  AND lark_thread_root_id = $3;
...
-- name: SetLarkChatSessionBindingIssue :exec
-- Attaches an issue to a thread binding once the dispatcher creates it
-- on the first @-mention of the thread. Guarded by issue_id IS NULL so a
-- replay or a race cannot overwrite an already-attached issue.
