...
func validateAndNormalizeResourceRef(resourceType string, ref json.RawMessage) (json.RawMessage, error) {
	...
	case "external_directory":
		return validateExternalDirectoryRef(ref)
	default:
...
// externalDirectoryRef stores an external path that agents may write to while
// keeping their working directory and durable context inside the Multica
// workspace. Reads are intentionally global; this resource is only the write
// allowlist.
...
func validateExternalDirectoryRef(ref json.RawMessage) (json.RawMessage, error) {
	var payload externalDirectoryRef
	if err := json.Unmarshal(ref, &payload); err != nil {
		return nil, fmt.Errorf("invalid external_directory payload: %w", err)
	}
	payload.LocalPath = strings.TrimSpace(payload.LocalPath)
	if payload.LocalPath == "" {
		return nil, errors.New("external_directory: local_path is required")
	}
	if !isAbsoluteLocalPath(payload.LocalPath) {
		return nil, errors.New("external_directory: local_path must be an absolute path")
	}
	payload.DaemonID = strings.TrimSpace(payload.DaemonID)
	if payload.DaemonID == "" {
		return nil, errors.New("external_directory: daemon_id is required")
	}
	payload.Label = strings.TrimSpace(payload.Label)
	out, err := json.Marshal(payload)
	if err != nil {
