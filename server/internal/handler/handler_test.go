...
func TestCreateIssueAdvancesStaleWorkspaceCounterPastMaxIssueNumber(t *testing.T) {
	ctx := context.Background()
	var highNumber int
	if err := testPool.QueryRow(ctx,
		`SELECT COALESCE(MAX(number), 0) + 25 FROM issue WHERE workspace_id = $1`,
		testWorkspaceID,
	).Scan(&highNumber); err != nil {
		t.Fatalf("read max issue number: %v", err)
	}
	var seededID string
	if err := testPool.QueryRow(ctx, `
		INSERT INTO issue (workspace_id, title, status, priority, creator_type, creator_id, number)
		VALUES ($1, 'stale counter seed', 'todo', 'none', 'member', $2, $3)
		RETURNING id
	`, testWorkspaceID, testUserID, highNumber).Scan(&seededID); err != nil {
		t.Fatalf("seed high-number issue: %v", err)
	}
	t.Cleanup(func() {
		testPool.Exec(ctx, `DELETE FROM issue WHERE id = $1`, seededID)
		testPool.Exec(ctx, `DELETE FROM issue WHERE title = 'stale counter create' AND workspace_id = $1`, testWorkspaceID)
	})
	if _, err := testPool.Exec(ctx, `UPDATE workspace SET issue_counter = 0 WHERE id = $1`, testWorkspaceID); err != nil {
		t.Fatalf("force stale counter: %v", err)
	}

	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/issues?workspace_id="+testWorkspaceID, map[string]any{
		"title":           "stale counter create",
		"allow_duplicate": true,
	})
	testHandler.CreateIssue(w, req)
	if w.Code != http.StatusCreated {
		t.Fatalf("CreateIssue with stale counter: expected 201, got %d: %s", w.Code, w.Body.String())
	}
	var created IssueResponse
	if err := json.NewDecoder(w.Body).Decode(&created); err != nil {
		t.Fatalf("decode created issue: %v", err)
	}
	if created.Number <= int32(highNumber) {
		t.Fatalf("created number = %d, want > %d", created.Number, highNumber)
	}
}
