...
const fixedVerificationCodeEnv = "MULTICA_FIXED_VERIFICATION_CODE"
const skipVerificationEmailEnv = "MULTICA_SKIP_VERIFICATION_EMAIL"
...
func generateCode() (string, error) {
	if fixedCode := strings.TrimSpace(os.Getenv(fixedVerificationCodeEnv)); fixedCode != "" {
		if !isSixDigitCode(fixedCode) {
			return "", fmt.Errorf("%s must be a six-digit code", fixedVerificationCodeEnv)
		}
...
func skipVerificationEmail() bool {
	return strings.EqualFold(strings.TrimSpace(os.Getenv(skipVerificationEmailEnv)), "true")
}
...
func (h *Handler) SendCode(w http.ResponseWriter, r *http.Request) {
	...
	if skipVerificationEmail() {
		slog.Warn("verification email skipped", "email", email)
	} else {
