		...
		if issue, err := h.Queries.GetIssue(r.Context(), task.IssueID); err == nil {
			...
			if issue.ProjectID.Valid {
				...
				if rows := h.listProjectResourcesForProject(r.Context(), issue.ProjectID); len(rows) > 0 {
					attachResources := shouldAttachProjectResourcesForTask(*task, issue)
					...
					for _, row := range rows {
						...
						// them as the issue's repos even when project_resources
						// themselves are intentionally hidden from the daemon.
...
func shouldAttachProjectResourcesForTask(task db.AgentTaskQueue, issue db.Issue) bool {
	if !task.ChatSessionID.Valid {
		return true
	}
	...
	return issue.OriginType.String != "lark_chat"
}

func (h *Handler) workflowContextForIssue(ctx context.Context, issueID pgtype.UUID) string {
	run, err := h.Queries.GetWorkflowRunByRootIssue(ctx, issueID)
	if err != nil {
		return ""
	}
	...
	_ = json.Unmarshal(run.NodesState, &nodes)

	var b strings.Builder
	fmt.Fprintf(&b, "Latest workflow run `%s`: status=%s", uuidToString(run.ID), run.Status)
	if run.CurrentNode != "" {
		fmt.Fprintf(&b, ", current_node=%s", run.CurrentNode)
	}
	if run.Error.Valid && strings.TrimSpace(run.Error.String) != "" {
		fmt.Fprintf(&b, ", error=%s", run.Error.String)
	}
	b.WriteString(".")
	if len(nodes) > 0 {
		b.WriteString("\nNodes:")
		...
		for _, key := range keys {
			node := nodes[key]
			fmt.Fprintf(&b, "\n- %s: %s", key, node.Status)
			if node.SubIssueID != nil && *node.SubIssueID != "" {
				fmt.Fprintf(&b, " (sub_issue_id=%s)", *node.SubIssueID)
			}
			if node.Error != nil && *node.Error != "" {
				fmt.Fprintf(&b, " error=%s", *node.Error)
			}
		}
	}
	return b.String()
