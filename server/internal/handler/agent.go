...
type AgentResponse struct {
	...
	// LarkIssueProjectID names the project where Lark group-chat threads
	// auto-create per-thread Issues. Empty disables the auto-issue feature
	// for this agent — group @-mentions still trigger chat tasks but no
	// Issue is created and follow-ups are not mirrored.
	LarkIssueProjectID *string `json:"lark_issue_project_id"`
...
func agentToResponse(a db.Agent) AgentResponse {
	...
	return AgentResponse{
		...
		LarkIssueProjectID: uuidToPtr(a.LarkIssueProjectID),
...
type CreateAgentRequest struct {
	...
	// LarkIssueProjectID, when non-nil and a valid UUID, opts this agent
	// into Lark group-chat auto-issue creation. Empty string clears it;
	// nil leaves it unset (== disabled).
	LarkIssueProjectID *string `json:"lark_issue_project_id"`
}
...
func (h *Handler) CreateAgent(w http.ResponseWriter, r *http.Request) {
	...
	larkProjectUUID := pgtype.UUID{}
	if req.LarkIssueProjectID != nil && *req.LarkIssueProjectID != "" {
		parsed, ok := parseUUIDOrBadRequest(w, *req.LarkIssueProjectID, "lark_issue_project_id")
		if !ok {
			return
		}
		larkProjectUUID = parsed
	}
	...
	created, err := h.Queries.CreateAgent(r.Context(), db.CreateAgentParams{
		...
		LarkIssueProjectID: larkProjectUUID,
	})
	if err != nil {
...
type UpdateAgentRequest struct {
	...
	ThinkingLevel *string `json:"thinking_level"`
	// LarkIssueProjectID is a tri-state field analogous to ThinkingLevel:
	//   - omitted → no change
	//   - "" → clear (disable auto-issue for this agent)
	//   - non-empty UUID → set to that project
	LarkIssueProjectID *string `json:"lark_issue_project_id"`
}
...
func (h *Handler) UpdateAgent(w http.ResponseWriter, r *http.Request) {
	...
	// lark_issue_project_id (tri-state): omitted = no change; "" = clear
	// (route through SetAgentLarkIssueProject post-update because the
	// COALESCE form cannot set the column to NULL); UUID = set.
	shouldClearLarkProject := false
	if req.LarkIssueProjectID != nil {
		value := *req.LarkIssueProjectID
		if value == "" {
			shouldClearLarkProject = true
		} else {
			parsed, ok := parseUUIDOrBadRequest(w, value, "lark_issue_project_id")
			if !ok {
				return
			}
			params.LarkIssueProjectID = parsed
		}
	}
	...
	if shouldClearLarkProject {
		updated, err = h.Queries.SetAgentLarkIssueProject(r.Context(), db.SetAgentLarkIssueProjectParams{
			ID:                 updated.ID,
			LarkIssueProjectID: pgtype.UUID{},
		})
		if err != nil {
			slog.Warn("clear agent lark_issue_project_id failed", append(logger.RequestAttrs(r), "error", err, "agent_id", id)...)
			writeError(w, http.StatusInternalServerError, "failed to clear lark_issue_project_id: "+err.Error())
			return
		}
	}
