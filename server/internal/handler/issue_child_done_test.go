...
func TestWorkflowManagedChildDoneDoesNotNotifyOrWakeParentAgent(t *testing.T) {
	fx := newChildDoneFixture(t, "in_progress")
	...
	if err := testPool.QueryRow(context.Background(), `
		...
		VALUES ($1, $2, 'running', 'implement', '{}'::jsonb, '{"meta":{"name":"child-done skip"}}'::jsonb)
		RETURNING id
	`, testWorkspaceID, fx.parent.ID).Scan(&runID); err != nil {
		t.Fatalf("create workflow run: %v", err)
	}
	t.Cleanup(func() {
		testPool.Exec(context.Background(), `DELETE FROM workflow_run WHERE id = $1`, runID)
	})
	if _, err := testPool.Exec(context.Background(),
		`UPDATE issue SET origin_type = 'workflow_node', origin_id = $1 WHERE id = $2`,
		runID, fx.child.ID,
	); err != nil {
		t.Fatalf("mark workflow-managed child: %v", err)
	}

	updateChildStatus(t, fx.child.ID, "done")

	if got := countSystemCommentsOn(t, fx.parent.ID); got != 0 {
		t.Errorf("workflow-managed child should not create parent system comment, got %d", got)
	}
	if got := countPendingTasksForAgent(t, fx.parent.ID, agentID); got != 0 {
		t.Errorf("workflow-managed child should not wake parent agent, got %d tasks", got)
	}
}
