...
func TestClaimTaskByRuntime_PopulatesWorkflowContext(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}

	ctx := context.Background()
	runtimeID := createClaimReclaimRuntime(t, ctx, "Workflow context claim runtime")
	agentID, issueID := createClaimReclaimAgentAndIssue(t, ctx, runtimeID, "Workflow context claim agent")
	taskID := createDispatchedClaimFixtureTask(t, ctx, agentID, runtimeID, issueID, "120 seconds", false)
	var runID string
	if err := testPool.QueryRow(ctx, `
		INSERT INTO workflow_run (workspace_id, root_issue_id, status, current_node, nodes_state, definition_snapshot)
		VALUES ($1, $2, 'done', 'END', '{"learn_node":{"status":"done","sub_issue_id":"child-learn"},"doc_node":{"status":"done","sub_issue_id":"child-doc"}}'::jsonb, '{"meta":{"name":"claim workflow"}}'::jsonb)
		RETURNING id
	...
	w := httptest.NewRecorder()
	req := newDaemonTokenRequest("POST", "/api/daemon/runtimes/"+runtimeID+"/tasks/claim", nil,
		testWorkspaceID, "workflow-context-claim")
	req = withURLParam(req, "runtimeId", runtimeID)
	testHandler.ClaimTaskByRuntime(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("ClaimTaskByRuntime: expected 200, got %d: %s", w.Code, w.Body.String())
	...
	for _, want := range []string{"status=done", "learn_node: done", "doc_node: done", "child-doc"} {
		if !strings.Contains(resp.Task.WorkflowContext, want) {
			t.Errorf("workflow_context missing %q: %s", want, resp.Task.WorkflowContext)
		}
	}
}
...
func TestClaimTask_LarkIssueChatSkipsProjectResourcesButKeepsProjectRepos(t *testing.T) {
	if testHandler == nil {
		t.Skip("database not available")
	}
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO project (workspace_id, title) VALUES ($1, $2) RETURNING id
	`, testWorkspaceID, "Lark issue chat project resources").Scan(&projectID); err != nil {
		t.Fatalf("create project: %v", err)
	}
	t.Cleanup(func() { testPool.Exec(context.Background(), `DELETE FROM project WHERE id = $1`, projectID) })

	const (
		projectRepoURL = "https://github.com/example/lark-project-repo"
		localDirPath   = "/Users/bytedance/Documents/bytedance/ai_workspace"
		localDaemonID  = "019ea78c-a85f-7a31-9dfa-0aeaaefd3883"
	)
	if _, err := testPool.Exec(ctx, `
		INSERT INTO project_resource (
			project_id, workspace_id, resource_type, resource_ref, position
	...
	`, projectID, testWorkspaceID,
		`{"url":"`+projectRepoURL+`"}`,
		`{"local_path":"`+localDirPath+`","daemon_id":"`+localDaemonID+`","label":"ai_workspace"}`); err != nil {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ($1, $2, $3, 'lark issue chat session')
		RETURNING id
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO issue (
			...
			creator_id, creator_type, number, position, origin_type, origin_id
		) VALUES ($1, $2, 'lark issue chat', 'in_progress', 'none',
			$3, 'member', 88003, 0, 'lark_chat', $4)
		RETURNING id
	`, testWorkspaceID, projectID, testUserID, chatSessionID).Scan(&issueID); err != nil {
		t.Fatalf("create issue: %v", err)
	}
	t.Cleanup(func() { testPool.Exec(context.Background(), `DELETE FROM issue WHERE id = $1`, issueID) })
	...
	if err := testPool.QueryRow(ctx, `
		...
		RETURNING id
	`, agentID, runtimeID, issueID, chatSessionID).Scan(&taskID); err != nil {
		t.Fatalf("create task: %v", err)
	}
	t.Cleanup(func() { testPool.Exec(context.Background(), `DELETE FROM agent_task_queue WHERE id = $1`, taskID) })

	w := httptest.NewRecorder()
	req := newDaemonTokenRequest("POST", "/api/daemon/runtimes/"+runtimeID+"/claim", nil, testWorkspaceID, "test-claim-lark-chat")
	req = withURLParam(req, "runtimeId", runtimeID)
	testHandler.ClaimTaskByRuntime(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("ClaimTaskByRuntime: %d %s", w.Code, w.Body.String())
	...
	if len(resp.Task.Repos) != 1 || resp.Task.Repos[0].URL != projectRepoURL {
		t.Fatalf("expected project github repo to remain available, got %+v", resp.Task.Repos)
	}
	if len(resp.Task.ProjectResources) != 0 {
		t.Fatalf("lark issue chat should not receive project_resources, got %+v", resp.Task.ProjectResources)
	}
}
...
func TestCompleteTask_IssueBackedChatPublishesChatDoneWithoutAssistantMessage(t *testing.T) {
	if testHandler == nil {
		t.Skip("database not available")
	}
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO issue (workspace_id, title, status, priority, creator_id, creator_type, number, position)
		VALUES ($1, 'issue-backed chat publish fixture', 'in_progress', 'none', $2, 'member', 81202, 0)
		RETURNING id
	`, testWorkspaceID, testUserID).Scan(&issueID); err != nil {
		t.Fatalf("setup: create issue: %v", err)
	}
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO chat_session (workspace_id, agent_id, creator_id, title)
		VALUES ($1, $2, $3, 'issue-backed lark thread')
		RETURNING id
	`, testWorkspaceID, agentID, testUserID).Scan(&chatSessionID); err != nil {
	...
	if err := testPool.QueryRow(ctx, `
		...
		RETURNING id
	`, agentID, runtimeID, issueID, chatSessionID).Scan(&taskID); err != nil {
		t.Fatalf("setup: create issue-backed chat task: %v", err)
	}
	t.Cleanup(func() { testPool.Exec(ctx, `DELETE FROM agent_task_queue WHERE id = $1`, taskID) })
	...
	const finalOutput = "来自 issue 任务的飞书回复"
	...
	case e := <-gotEvent:
		...
		if !ok {
			t.Fatalf("chat:done payload type = %T, want protocol.ChatDonePayload", e.Payload)
		}
		if payload.ChatSessionID != chatSessionID {
			t.Fatalf("chat:done chat_session_id = %q, want %q", payload.ChatSessionID, chatSessionID)
		}
		if payload.TaskID != taskID {
			t.Fatalf("chat:done task_id = %q, want %q", payload.TaskID, taskID)
		}
		if payload.Content != finalOutput {
			t.Fatalf("chat:done content = %q, want %q", payload.Content, finalOutput)
		}
		if payload.MessageID != "" {
			t.Fatalf("issue-backed chat task must not create assistant chat_message, got message_id=%q", payload.MessageID)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("expected chat:done event for issue-backed chat task")
	}

	var assistantCount int
	if err := testPool.QueryRow(ctx, `
		SELECT count(*) FROM chat_message WHERE chat_session_id = $1 AND role = 'assistant'
	`, chatSessionID).Scan(&assistantCount); err != nil {
		t.Fatalf("count assistant chat messages: %v", err)
	}
	if assistantCount != 0 {
		t.Fatalf("issue-backed chat task must not persist assistant chat_message, got %d", assistantCount)
	}
}
