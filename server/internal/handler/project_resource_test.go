...
func TestProjectResourceExternalDirectoryLifecycle(t *testing.T) {
	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/projects?workspace_id="+testWorkspaceID, map[string]any{
		"title": "External directory project",
	})
	testHandler.CreateProject(w, req)
	if w.Code != http.StatusCreated {
		t.Fatalf("CreateProject: %d %s", w.Code, w.Body.String())
	...
	w = httptest.NewRecorder()
	req = newRequest("POST", "/api/projects/"+project.ID+"/resources", map[string]any{
		"resource_type": "external_directory",
		"resource_ref": map[string]any{
			"local_path": "/Users/foo/Documents/source-notes",
			"daemon_id":  "daemon-simultica",
			"label":      "Source Notes",
		},
	...
	if created.ResourceType != "external_directory" {
		t.Errorf("ResourceType = %q, want external_directory", created.ResourceType)
	}
	var ref struct {
		LocalPath string `json:"local_path"`
		DaemonID  string `json:"daemon_id"`
	...
	if ref.LocalPath != "/Users/foo/Documents/source-notes" || ref.DaemonID != "daemon-simultica" || ref.Label != "Source Notes" {
		t.Errorf("ref = %+v", ref)
	}
}

func TestProjectResourceExternalDirectoryValidation(t *testing.T) {
	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/projects?workspace_id="+testWorkspaceID, map[string]any{
		"title": "External directory validation",
	})
	testHandler.CreateProject(w, req)
	if w.Code != http.StatusCreated {
		t.Fatalf("CreateProject: %d %s", w.Code, w.Body.String())
	...
	cases := []struct {
		...
		{"missing local_path", map[string]any{"daemon_id": "d1"}},
		{"blank local_path", map[string]any{"local_path": "   ", "daemon_id": "d1"}},
		{"relative local_path", map[string]any{"local_path": "notes", "daemon_id": "d1"}},
		{"home-shorthand path", map[string]any{"local_path": "~/notes", "daemon_id": "d1"}},
		{"missing daemon_id", map[string]any{"local_path": "/Users/foo/notes"}},
		{"blank daemon_id", map[string]any{"local_path": "/Users/foo/notes", "daemon_id": ""}},
		{"wrong type in payload", map[string]any{"local_path": 42, "daemon_id": "d1"}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
