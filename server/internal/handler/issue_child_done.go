...
func (h *Handler) notifyParentOfChildDone(ctx context.Context, prev, issue db.Issue, actorType, actorID string) {
	...
	if isWorkflowManagedIssue(issue) {
		return
	}
...
func isWorkflowManagedIssue(issue db.Issue) bool {
	return issue.OriginType.Valid && issue.OriginType.String == "workflow_node" && issue.OriginID.Valid
}
