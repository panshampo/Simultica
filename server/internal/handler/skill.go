...
func mergeSkillConfigFlag(raw []byte, hasWorkflow bool) ([]byte, error) {
	return mergeSkillConfigWorkflow(raw, hasWorkflow, nil)
}

func mergeSkillConfigWorkflow(raw []byte, hasWorkflow bool, validation any) ([]byte, error) {
	cfg := map[string]any{}
	if len(raw) > 0 {
	...
	cfg["has_workflow"] = hasWorkflow
	if validation != nil {
		cfg["workflow_validation"] = validation
	} else if !hasWorkflow {
		delete(cfg, "workflow_validation")
	}
	return json.Marshal(cfg)
}

func (h *Handler) setSkillHasWorkflow(ctx context.Context, skill db.Skill, hasWorkflow bool) (db.Skill, error) {
	config, err := mergeSkillConfigWorkflow(skill.Config, hasWorkflow, nil)
	if err != nil {
		return skill, err
	}
	return h.Queries.UpdateSkill(ctx, db.UpdateSkillParams{
		ID:     skill.ID,
		Config: config,
	})
}
...
type skillWorkflowValidationResult struct {
	Valid    bool     `json:"valid"`
	Errors   []string `json:"errors"`
	Warnings []string `json:"warnings"`
}

type skillWorkflowDefinition struct {
	Meta      map[string]any       `yaml:"meta"`
	State     skillWorkflowState   `yaml:"state"`
	Nodes     []skillWorkflowNode  `yaml:"nodes"`
	Routing   []skillWorkflowRoute `yaml:"routing"`
	Execution map[string]any       `yaml:"execution"`
}

type skillWorkflowState struct {
	Fields []skillWorkflowField `yaml:"fields"`
}

type skillWorkflowField struct {
	Name string `yaml:"name"`
	Type string `yaml:"type"`
}

type skillWorkflowNode struct {
	ID         string                          `yaml:"id"`
	Type       string                          `yaml:"type"`
	OnComplete []skillWorkflowOnCompleteAction `yaml:"on_complete"`
}

type skillWorkflowOnCompleteAction struct {
	Action string `yaml:"action"`
	Field  string `yaml:"field"`
}

type skillWorkflowRoute struct {
...
func validateSkillWorkflowYAML(content string) skillWorkflowValidationResult {
	result := skillWorkflowValidationResult{Valid: true, Errors: []string{}, Warnings: []string{}}
	var def skillWorkflowDefinition
	if err := yaml.Unmarshal([]byte(content), &def); err != nil {
		result.Valid = false
		result.Errors = append(result.Errors, "workflow.yaml must be valid YAML: "+err.Error())
		return result
	}
	...
	for _, field := range def.State.Fields {
		if field.Name != "" {
			stateTypes[field.Name] = field.Type
		}
	}
	...
	for i, node := range def.Nodes {
		...
		for _, action := range node.OnComplete {
			if action.Action == "increment" && stateTypes[action.Field] != "number" {
				result.Errors = append(result.Errors, "node "+node.ID+" on_complete increment field "+action.Field+" must be a number state field")
			}
		}
	}
	if len(def.Nodes) == 0 {
		result.Errors = append(result.Errors, "workflow.nodes must contain at least one node")
	}
	if len(def.Routing) == 0 {
		result.Errors = append(result.Errors, "workflow.routing must contain at least one route")
	}

	hasMaxSteps := hasPositiveNumber(def.Execution["maxSteps"])
	for _, route := range def.Routing {
		if route.Condition != "" {
			if route.Else == "" {
				result.Errors = append(result.Errors, "conditional route from "+route.From+" must define else")
			}
			for _, field := range extractWorkflowConditionFields(route.Condition) {
				if _, ok := stateTypes[field]; !ok {
					result.Errors = append(result.Errors, "route from "+route.From+" condition references undefined state field: "+field)
				}
			}
		}
		if isSkillWorkflowLoopBack(route, nodeOrder) && !hasMaxSteps {
			result.Errors = append(result.Errors, "loop-back route "+route.From+" -> "+route.To+" requires execution.maxSteps")
		}
	}
...
func hasPositiveNumber(v any) bool {
	switch n := v.(type) {
	case int:
		return n > 0
	case int64:
...
func isSkillWorkflowLoopBack(route skillWorkflowRoute, nodeOrder map[string]int) bool {
	if route.From == "" || route.To == "" || route.From == "START" || route.To == "END" {
		return false
	}
	from, okFrom := nodeOrder[route.From]
	to, okTo := nodeOrder[route.To]
	return okFrom && okTo && to <= from
}

func extractWorkflowConditionFields(condition string) []string {
	condition = stripQuotedWorkflowConditionValues(condition)
	seen := map[string]bool{}
	fields := []string{}
	for _, token := range strings.FieldsFunc(condition, func(r rune) bool {
...
func stripQuotedWorkflowConditionValues(condition string) string {
