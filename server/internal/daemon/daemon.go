...
var (
	...
	// helpers above.
	detectAgentVersion = func(ctx context.Context, path string, args []string) (string, error) {
		return agent.DetectVersionWithArgs(ctx, path, args)
	}
...
func (d *Daemon) registerRuntimesForWorkspace(ctx context.Context, workspaceID string) (*RegisterResponse, error) {
	...
	for name, entry := range d.cfg.Agents {
		version, err := detectAgentVersion(ctx, entry.Path, entry.Args)
		if err != nil {
...
func (d *Daemon) handleTask(ctx context.Context, task Task, slot int) {
	...
	if result.Status == "completed" {
		d.uploadFinalImageAttachments(ctx, task.ID, result.WorkDir, result.Comment, taskLog)
	}
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (TaskResult, error) {
	...
	taskCtx := execenv.TaskContextForEnv{
		...
		WorkflowMainNode:                 convertWorkflowMainNodeForEnv(task.WorkflowMainNode),
...
func convertWorkflowMainNodeForEnv(in *WorkflowMainNodeData) *execenv.WorkflowMainNodeForEnv {
	if in == nil {
		return nil
	}
	return &execenv.WorkflowMainNodeForEnv{RunID: in.RunID, NodeID: in.NodeID, NodeType: in.NodeType}
}
