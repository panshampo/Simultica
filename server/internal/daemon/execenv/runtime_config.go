...
func formatProjectResource(r ProjectResourceForEnv) string {
	...
	case "external_directory":
		...
		_ = json.Unmarshal(r.ResourceRef, &payload)
		out := fmt.Sprintf("**External writable directory**: `%s`", payload.LocalPath)
		if payload.Label != "" {
			out += fmt.Sprintf(" (`%s`)", payload.Label)
		}
		if payload.DaemonID != "" {
			out += fmt.Sprintf(" on daemon `%s`", payload.DaemonID)
		}
		out += "; this authorizes writes there but keeps the task working directory inside Multica."
		if label != "" {
			out += " — " + label
		}
		return out
...
// For DS Copilot: writes {workDir}/CLAUDE.md  (Claude Code wrapper with CLAUDE_CONFIG_DIR=~/.ds-copilot)
...
func runtimeConfigPath(workDir, provider string) string {
	switch provider {
	case "claude", "ds-copilot":
		return filepath.Join(workDir, "CLAUDE.md")
	case "codex", "copilot", "opencode", "openclaw", "hermes", "pi", "cursor", "kimi", "kiro", "antigravity", "traex":
...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if wfText := strings.TrimRight(ctx.WorkflowContext, " \t\r\n"); wfText != "" {
		b.WriteString("## Runtime Workflow Context\n\n")
		b.WriteString(wfText)
		b.WriteString("\n\n")
	}
	...
	if ctx.IssueID != "" && ctx.ChatSessionID == "" && ctx.AutopilotRunID == "" && ctx.QuickCreatePrompt == "" {
		...
		b.WriteString("5. Convert the YAML definition to JSON, then start the workflow through the WorkflowCase control plane:\n\n")
		b.WriteString("```\n")
		b.WriteString("multica workflow-case create --entry-issue <issueId> --output json\n")
		b.WriteString("multica workflow-case definition upsert <caseId> --definition-stdin\n")
		b.WriteString("multica workflow-case definition validate <caseId> --output json\n")
		b.WriteString("multica workflow-case definition publish <caseId> --output json\n")
		b.WriteString("multica workflow-case run start <caseId> --run-kind primary --output json\n")
		b.WriteString("```\n\n")
		b.WriteString("Use `create` to create or find a WorkflowCase for this entry issue, `definition upsert` to save the draft, `definition validate` to check it, `definition publish` to freeze the draft into the case's online version, and `run start` to start the WorkflowRun from that online version. Publish online version does not start a run. Start run always uses the online version, so do NOT pass a definition version id. Paste the full workflow JSON into stdin for the upsert command. After run start succeeds, your task is complete for now. Immediately mark the issue as `blocked` with reason `waiting for runtime workflow`, then exit. Do NOT inspect code, poll child issues, create fallback sub-issues, write findings, or produce a final answer. The sidecar will dispatch carrier nodes to sub-agents, and a final main_issue_task node will trigger you again for review/summary.\n\n")
	...
	b.WriteString("Global filesystem reads are allowed for local investigation. Keep writes inside the Multica workspace unless an external path is explicitly listed for this task; write to external paths only when they appear as `external_directory` resources. An `external_directory` resource is a write allowlist entry and does not change your current working directory or session context.\n\n")
	...
	if len(ctx.AgentSkills) > 0 {
		...
		case "codex", "copilot", "opencode", "openclaw", "pi", "cursor", "kimi", "kiro", "antigravity", "traex":
	...
	// Image Output contract — taught to the agent so it can emit chart / diagram
	// images via plain markdown links and have the daemon redeliver them as
	// platform-native image attachments. Skipped for quick-create (no comment
	// to attach to) and run-only autopilot (no issue, captured-stdout only).
	if ctx.QuickCreatePrompt == "" && ctx.AutopilotRunID == "" {
		b.WriteString("## Image Output\n\n")
		b.WriteString("You can attach images (charts, diagrams, screenshots) to your final comment by referencing them in markdown. The daemon scans your final answer for `![alt](path)` links, uploads matching local files, and the platform redelivers them on Lark/web/desktop.\n\n")
		b.WriteString("- Generate or save images to a path **inside the task workdir**, then reference them with a **workdir-relative** markdown link, e.g. `![chart](./out/cpu.png)`. Use the `multica-chart-render` skill when you need to plot data.\n")
		b.WriteString("- Only `.png`, `.jpg`, `.jpeg` are supported. Each image must be **≤ 10 MiB**; oversized files are silently dropped.\n")
		b.WriteString("- Paths must stay inside the workdir — absolute paths and `..` traversal are rejected.\n")
		b.WriteString("- Only images you actually reference in the final answer are uploaded; unreferenced files on disk are ignored.\n")
		b.WriteString("- Remote `http(s)://` image links pass through untouched — do not download and re-reference them.\n")
		b.WriteString("- Lark cards display **at most 5 images per comment**; additional images are truncated with a notice. Keep image counts small.\n")
		b.WriteString("- Upload is best-effort: if every image fails to upload, the comment degrades to text-only and the run does not fail.\n\n")
	}
