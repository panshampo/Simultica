...
func TestPrepareWithProjectResources(t *testing.T) {
	...
	taskCtx := TaskContextForEnv{
		...
		ProjectResources: []ProjectResourceForEnv{
			...
			},
			{
				...
				ResourceRef:  json.RawMessage(`{"local_path":"/Users/foo/Documents/source-notes","daemon_id":"daemon-simultica","label":"Source Notes"}`),
				Label:        "Writable notes",
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	if len(got.Resources) != 2 || got.Resources[0].ResourceType != "github_repo" || got.Resources[1].ResourceType != "external_directory" {
	...
	for _, want := range []string{
		...
		"default branch: `main`",
		"Global filesystem reads are allowed",
		"write to external paths only when they appear as `external_directory` resources",
		"External writable directory",
		"/Users/foo/Documents/source-notes",
		"keeps the task working directory inside Multica",
...
func TestBriefAlwaysCarriesFilesystemAccessPolicy(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	...
	for _, want := range []string{
		...
		"Global filesystem reads are allowed",
		"write to external paths only when they appear as `external_directory` resources",
		"does not change your current working directory or session context",
	} {
		if !strings.Contains(s, want) {
			t.Errorf("CLAUDE.md missing %q", want)
		}
...
func TestPrepareCodexHomeAddsExternalWritableRoots(t *testing.T) {
	t.Setenv("CODEX_HOME", t.TempDir())
	codexHome := filepath.Join(t.TempDir(), "codex-home")

	if err := prepareCodexHomeWithOpts(codexHome, CodexHomeOptions{
		GOOS:          "linux",
		WritableRoots: []string{"/Users/foo/Documents", "/Users/foo/Code"},
	}, testLogger()); err != nil {
	...
	s := string(data)
	if !strings.Contains(s, `sandbox_workspace_write.writable_roots = ["/Users/foo/Documents","/Users/foo/Code"]`) {
		t.Fatalf("config.toml missing writable_roots, got:\n%s", s)
	}
}
