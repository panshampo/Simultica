...
func TestWorkflowContextRenderedForIssueTasks(t *testing.T) {
	t.Parallel()
	const wfContext = "Latest workflow run: done. learn_node done -> sub-issue PXB-10. doc_node done -> sub-issue PXB-11."
	cases := []struct {
		name string
		ctx  TaskContextForEnv
	}{
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			out := buildMetaSkillContent("claude", tc.ctx)
			for _, want := range []string{"## Runtime Workflow Context", "Latest workflow run: done", "learn_node done", "doc_node done"} {
				if !strings.Contains(out, want) {
					t.Errorf("[%s] workflow context missing %q\n---\n%s", tc.name, want, out)
				}
			}
		})
	}
...
// Image Output section teaches the agent the contract enforced by
// daemon/attachment_scan.go (workdir-relative `![](...)` links, PNG/JPG only,
// 10 MiB cap, no `..`, only-referenced-files-uploaded) plus the Lark-side
// 5-image cap from outbound.go. Must render for every task kind that ends
// in a real comment, and skip kinds that have no comment surface
// (quick-create, run-only autopilot).
func TestImageOutputSectionRenderedForCommentingTaskKinds(t *testing.T) {
	t.Parallel()
	cases := []struct {
		name string
		ctx  TaskContextForEnv
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			out := buildMetaSkillContent("claude", tc.ctx)
			if !strings.Contains(out, "## Image Output") {
				t.Fatalf("[%s] expected `## Image Output` heading", tc.name)
			}
			for _, want := range []string{
				"![chart](./out/cpu.png)",
				"`.png`, `.jpg`, `.jpeg`",
				...
				"workdir",
				"`..`",
				"`multica-chart-render`",
				"at most 5 images per comment",
				"degrades to text-only",
			} {
				if !strings.Contains(out, want) {
					t.Errorf("[%s] Image Output section missing %q", tc.name, want)
				}
			}
			// The Image Output section must sit between Attachments and the
			// "Always Use the multica CLI" guardrail so it reads as part of
			// the comment-output contract instead of the trigger workflow.
			attachIdx := strings.Index(out, "## Attachments")
			imgIdx := strings.Index(out, "## Image Output")
			cliIdx := strings.Index(out, "## Important: Always Use the `multica` CLI")
			if attachIdx == -1 || imgIdx == -1 || cliIdx == -1 || !(attachIdx < imgIdx && imgIdx < cliIdx) {
				t.Errorf("[%s] expected ordering Attachments < Image Output < Important (got %d, %d, %d)", tc.name, attachIdx, imgIdx, cliIdx)
			}
		})
	}
}

func TestImageOutputSectionSkippedForCommentlessTaskKinds(t *testing.T) {
	t.Parallel()
	cases := []struct {
		name string
		ctx  TaskContextForEnv
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			out := buildMetaSkillContent("claude", tc.ctx)
			if strings.Contains(out, "## Image Output") {
				t.Errorf("[%s] task kind has no comment surface; Image Output section must be skipped", tc.name)
			}
		})
	}
}
