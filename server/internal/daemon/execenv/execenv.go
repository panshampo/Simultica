...
type PrepareParams struct {
	...
	LocalWorkDir string
	// DaemonID scopes machine-local project resources. external_directory
	// resources only become writable_roots when their daemon_id matches this
	// daemon; reads remain globally allowed and do not depend on this.
	DaemonID string
...
func Prepare(params PrepareParams, logger *slog.Logger) (*Environment, error) {
	...
	if params.Provider == "codex" {
		...
		if err := prepareCodexHomeWithOpts(codexHome, CodexHomeOptions{
			...
			WritableRoots: externalWritableRoots(params.Task.ProjectResources, params.DaemonID),
		}, logger); err != nil {
