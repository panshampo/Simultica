...
type codexSandboxPolicy struct {
	...
	Reason string
	// WritableRoots are extra absolute paths the agent may write when Mode is
	// workspace-write. They are ignored by danger-full-access because that mode
	// has no filesystem sandbox to narrow.
...
func renderMulticaManagedBlock(policy codexSandboxPolicy) string {
	...
	if policy.Mode == "workspace-write" {
		...
		if len(policy.WritableRoots) > 0 {
			roots, _ := json.Marshal(policy.WritableRoots)
			b.WriteString(fmt.Sprintf("sandbox_workspace_write.writable_roots = %s\n", roots))
		}
	}
