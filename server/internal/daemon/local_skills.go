...
//   - DS Copilot: ~/.ds-copilot/skills (launcher syncs user skills there)
...
func localSkillRootForProvider(provider string) (string, bool, error) {
	...
	case "ds-copilot":
		return filepath.Join(home, ".ds-copilot", "skills"), true, nil
	case "codex":
	...
	case "traex":
		...
		return filepath.Join(traeHome, "skills"), true, nil
	default:
		return "", false, nil
	}
}
