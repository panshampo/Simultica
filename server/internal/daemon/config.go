...
type Config struct {
	...
	Agents                         map[string]AgentEntry // keyed by provider: claude, ds-copilot, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor, kimi, kiro, antigravity, traex
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	probeDsCopilot := func() (AgentEntry, bool) {
		cmd := envOrDefault("MULTICA_DS_COPILOT_PATH", "ds-copilot")
		if e, ok := probe("MULTICA_DS_COPILOT_PATH", "ds-copilot", ""); ok {
			return e, true
		}
		if strings.ContainsAny(cmd, "/\\") {
			return dsCopilotLaunchScriptEntry(cmd)
		}
		if cmd == "ds-copilot" {
			return defaultDsCopilotLaunchScriptEntry()
		}
		return AgentEntry{}, false
	}
	...
	// Traex is a Codex fork (`traecli`); the daemon talks to it via the
	// shared Codex `app-server --listen stdio://` JSON-RPC entry point.
	// MULTICA_TRAEX_MODEL seeds the workspace-wide default model name when
	// the agent has no explicit `model` set.
	if e, ok := probe("MULTICA_TRAEX_PATH", "traex", "MULTICA_TRAEX_MODEL"); ok {
		agents["traex"] = e
	}
	if len(agents) == 0 {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, ds-copilot, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor-agent, kimi, kiro-cli, agy, or traex and ensure it is on PATH")
	}
...
var defaultAgentCommandNames = []string{
	"claude", "ds-copilot", "codex", "opencode", "openclaw", "hermes",
	"gemini", "pi", "cursor-agent", "copilot", "kimi", "kiro-cli", "agy", "traex",
}
...
func defaultDsCopilotLaunchScriptEntry() (AgentEntry, bool) {
	home, err := os.UserHomeDir()
	if err != nil || strings.TrimSpace(home) == "" {
		return AgentEntry{}, false
	}
	return dsCopilotLaunchScriptEntry(filepath.Join(home, ".ds-copilot", "launch.sh"))
}

func dsCopilotLaunchScriptEntry(scriptPath string) (AgentEntry, bool) {
	scriptPath = expandPathEnv(scriptPath)
	info, err := os.Stat(scriptPath)
	if err != nil || info.IsDir() || !info.Mode().IsRegular() {
		return AgentEntry{}, false
	}
	bashPath, ok := resolveBashPath()
	if !ok {
		return AgentEntry{}, false
	}
	return AgentEntry{
...
func expandPathEnv(path string) string {
...
func resolveBashPath() (string, bool) {
	if path, err := exec.LookPath("bash"); err == nil {
		return path, true
	}
	for _, path := range []string{"/bin/bash", "/usr/bin/bash"} {
