...
func TestListRuntimeLocalSkills_DsCopilot(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)

	writeTestLocalSkill(t, filepath.Join(home, ".ds-copilot", "skills"), "ds-query", map[string]string{
		"SKILL.md": "---\nname: DS Query\ndescription: Query internal data\n---\n# DS Query\n",
	})

	skills, supported, err := listRuntimeLocalSkills("ds-copilot")
	if err != nil {
		t.Fatalf("listRuntimeLocalSkills: %v", err)
	}
	if !supported {
		t.Fatal("ds-copilot should be supported")
	}
	if len(skills) != 1 {
		t.Fatalf("expected 1 skill, got %d", len(skills))
	}
	if skills[0].Key != "ds-query" {
		t.Fatalf("key = %q, want ds-query", skills[0].Key)
	}
	if skills[0].SourcePath != "~/.ds-copilot/skills/ds-query" {
