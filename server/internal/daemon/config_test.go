...
func TestLoadConfig_DetectsDsCopilotExplicitPath(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("POSIX shell not available on Windows")
	}

	pathDir := t.TempDir()
	fakeDS := filepath.Join(pathDir, "ds-copilot")
	if err := os.WriteFile(fakeDS, []byte("#!/bin/sh\nexit 0\n"), 0o755); err != nil {
		t.Fatalf("write fake ds-copilot: %v", err)
	}

	missingDir := t.TempDir()
	t.Setenv("PATH", missingDir)
	t.Setenv("SHELL", filepath.Join(missingDir, "fish"))
	t.Setenv("MULTICA_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	t.Setenv("MULTICA_CODEX_PATH", filepath.Join(missingDir, "missing-codex"))
	pinNonCodexAgentsToMissingPaths(t)
	t.Setenv("MULTICA_DS_COPILOT_PATH", fakeDS)
	...
	got, ok := cfg.Agents["ds-copilot"]
	if !ok {
		t.Fatalf("expected ds-copilot agent, got %#v", cfg.Agents)
	}
	if got.Path != fakeDS {
		t.Fatalf("ds-copilot path = %q, want %q", got.Path, fakeDS)
	}
	if len(got.Args) != 0 {
		t.Fatalf("ds-copilot executable wrapper should not get prefix args, got %v", got.Args)
	}
	if got.Model != "" {
		t.Fatalf("ds-copilot model override should stay empty because model selection is launcher-managed, got %q", got.Model)
	}
}

func TestLoadConfig_DetectsDsCopilotDefaultLaunchScript(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("bash launcher fallback is POSIX-only")
	}

	home := t.TempDir()
	launchScript := filepath.Join(home, ".ds-copilot", "launch.sh")
	if err := os.MkdirAll(filepath.Dir(launchScript), 0o755); err != nil {
		t.Fatalf("mkdir ds-copilot dir: %v", err)
	}
	if err := os.WriteFile(launchScript, []byte("echo 1.8.2\n"), 0o644); err != nil {
		t.Fatalf("write ds-copilot launcher: %v", err)
	}
	...
	t.Setenv("SHELL", filepath.Join(missingDir, "fish"))
	t.Setenv("MULTICA_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	pinAgentPathsToMissingExcept(t, "MULTICA_DS_COPILOT_PATH")
	wantBash, ok := resolveBashPath()
	if !ok {
	...
	got, ok := cfg.Agents["ds-copilot"]
	if !ok {
		t.Fatalf("expected ds-copilot agent from launch.sh fallback, got %#v", cfg.Agents)
	}
	if got.Path != wantBash {
		t.Fatalf("ds-copilot path = %q, want bash path %q", got.Path, wantBash)
	}
	if !reflect.DeepEqual(got.Args, []string{launchScript}) {
		t.Fatalf("ds-copilot args = %v, want [%q]", got.Args, launchScript)
	}
}

func TestLoadConfig_DetectsDsCopilotExplicitLaunchScriptPath(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("bash launcher fallback is POSIX-only")
	}
	...
	t.Setenv("MULTICA_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	pinAgentPathsToMissingExcept(t, "MULTICA_DS_COPILOT_PATH")
	t.Setenv("MULTICA_DS_COPILOT_PATH", launchScript)
	wantBash, ok := resolveBashPath()
	if !ok {
		t.Skip("bash not available")
	}
	...
	got, ok := cfg.Agents["ds-copilot"]
	if !ok {
		t.Fatalf("expected ds-copilot agent from explicit launch.sh, got %#v", cfg.Agents)
	}
	if got.Path != wantBash {
		t.Fatalf("ds-copilot path = %q, want bash path %q", got.Path, wantBash)
	}
...
func pinNonCodexAgentsToMissingPaths(t *testing.T) {
	t.Helper()
	pinAgentPathsToMissingExcept(t, "MULTICA_CODEX_PATH")
}

func pinAgentPathsToMissingExcept(t *testing.T, except ...string) {
	t.Helper()
	missingDir := t.TempDir()
