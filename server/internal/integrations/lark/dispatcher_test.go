...
type fakeQueries struct {
	...
	calledMark            int
	calledRelease         int
	bindingIssueSet       db.SetLarkChatSessionBindingIssueParams
	bindingIssueSetCalled int
	bindingIssueErr       error
	commentsCreated       []db.CreateCommentParams
	createCommentErr      error
	metadataSet           []db.SetIssueMetadataKeyParams
	metadataSetErr        error
}
...
func (f *fakeQueries) GetAgent(ctx context.Context, id pgtype.UUID) (db.Agent, error) {
...
func (f *fakeQueries) SetLarkChatSessionBindingIssue(ctx context.Context, arg db.SetLarkChatSessionBindingIssueParams) error {
	f.bindingIssueSet = arg
	f.bindingIssueSetCalled++
	return f.bindingIssueErr
}
...
func (f *fakeQueries) CreateComment(ctx context.Context, arg db.CreateCommentParams) (db.Comment, error) {
	...
	return db.Comment{ID: validUUID(0xC0 + byte(len(f.commentsCreated))), IssueID: arg.IssueID, AuthorType: arg.AuthorType, AuthorID: arg.AuthorID, Content: arg.Content, Type: arg.Type, WorkspaceID: arg.WorkspaceID}, nil
}

func (f *fakeQueries) SetIssueMetadataKey(ctx context.Context, arg db.SetIssueMetadataKeyParams) (db.Issue, error) {
	f.metadataSet = append(f.metadataSet, arg)
	if f.metadataSetErr != nil {
		return db.Issue{}, f.metadataSetErr
	}
	return db.Issue{ID: arg.ID, WorkspaceID: arg.WorkspaceID, Metadata: []byte(`{"` + arg.Key + `":` + string(arg.Value) + `}`)}, nil
}
...
type fakeChat struct {
	ensureID         pgtype.UUID // shorthand for ensureResult.ChatSessionID
	ensureResult     EnsureChatSessionResult
	ensureErr        error
...
func (f *fakeChat) EnsureChatSession(ctx context.Context, p EnsureChatSessionParams) (EnsureChatSessionResult, error) {
	f.calledEnsure++
	f.lastEnsureParams = p
	res := f.ensureResult
	if !res.ChatSessionID.Valid && f.ensureID.Valid {
		res.ChatSessionID = f.ensureID
	}
	return res, f.ensureErr
}
...
type fakeIssueCreator struct {
	...
	paramsHistory []service.IssueCreateParams
	result        service.IssueCreateResult
	err           error
	create        func(context.Context, service.IssueCreateParams, service.IssueCreateOpts) (service.IssueCreateResult, error)
}
...
func (f *fakeIssueCreator) Create(ctx context.Context, p service.IssueCreateParams, opts service.IssueCreateOpts) (service.IssueCreateResult, error) {
	f.called++
	f.params = p
	f.paramsHistory = append(f.paramsHistory, p)
	if f.create != nil {
		return f.create(ctx, p, opts)
	}
...
type fakeEnqueuer struct {
	calledIssueChat  int
	issueChatIssue   db.Issue
	issueChatSession pgtype.UUID
	called           int
...
func (f *fakeEnqueuer) EnqueueTaskForIssueChat(ctx context.Context, issue db.Issue, chatSessionID pgtype.UUID) (db.AgentTaskQueue, error) {
	f.calledIssueChat++
	f.issueChatIssue = issue
	f.issueChatSession = chatSessionID
	return f.task, f.err
}
...
func assertSenderOpenIDMetadata(t *testing.T, queries *fakeQueries, issueID pgtype.UUID, want string) {
	t.Helper()
	for _, arg := range queries.metadataSet {
		if arg.ID == issueID && arg.Key == metadataLarkSenderOpenID {
			if got := string(arg.Value); got != `"`+want+`"` {
				t.Fatalf("sender open_id metadata value = %s, want %q", got, want)
			}
			return
		}
	}
	t.Fatalf("missing %s metadata write for issue %+v; writes=%+v", metadataLarkSenderOpenID, issueID, queries.metadataSet)
}
...
func TestDispatcher_UnboundUserFallsBackToInstaller(t *testing.T) {
	inst := activeInstallation()
	...
	if res.Outcome != OutcomeIngested {
		t.Fatalf("unbound user should still ingest, got %q", res.Outcome)
	}
	if chat.lastEnsureParams.Sender != inst.InstallerUserID {
		t.Fatalf("unbound p2p sender should fall back to installer; got %+v want %+v",
			chat.lastEnsureParams.Sender, inst.InstallerUserID)
	}
	if chat.lastAppendParams.Sender != inst.InstallerUserID {
		t.Fatalf("unbound append sender should fall back to installer; got %+v",
			chat.lastAppendParams.Sender)
	}
	if len(audit.drops) != 0 {
		t.Fatalf("unbound user must no longer produce a drop audit row, got %+v", audit.drops)
	}
}
...
// the binding lookup: a replayed event for an open_id must not re-run
// the binding query on every reconnect — dedup short-circuits before
// any identity work happens.
...
func TestDispatcher_IssueCommandCreatesIssue(t *testing.T) {
	...
	if !issueSvc.params.AllowDuplicate {
		t.Fatalf("Lark /issue commands must allow duplicate titles: %+v", issueSvc.params)
	}
	...
	assertSenderOpenIDMetadata(t, queries, validUUID(0x88), "ou_user_a")
}
...
func TestDispatcher_GroupMentionAutoCreatesIssueAndMirrorsUserComment(t *testing.T) {
	sessionID := validUUID(0x66)
	bindingID := validUUID(0x67)
	issueID := validUUID(0x88)
	inst := activeInstallation()
	sender := boundUser()
	projectID := validUUID(0x99)
	queries := &fakeQueries{
		installationByApp: inst,
		userBinding:       sender,
		chatSession:       db.ChatSession{ID: sessionID, AgentID: inst.AgentID},
		agent:             db.Agent{ID: inst.AgentID, LarkIssueProjectID: projectID},
	}
	chat := &fakeChat{
		ensureResult: EnsureChatSessionResult{
	...
	res, err := d.Handle(context.Background(), InboundMessage{
		...
		CommandBody:    "check the dashboard",
		MessageID:      "msg-root",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	if issueSvc.called != 1 {
		t.Fatalf("expected one auto issue create, got %d", issueSvc.called)
	}
	if issueSvc.params.ProjectID != projectID {
		t.Fatalf("auto issue project mismatch: got %+v want %+v", issueSvc.params.ProjectID, projectID)
	}
	if issueSvc.params.OriginType.String != originLarkChat || issueSvc.params.OriginID != sessionID {
		t.Fatalf("auto issue origin mismatch: %+v", issueSvc.params)
	}
	if queries.bindingIssueSetCalled != 1 || queries.bindingIssueSet.ID != bindingID || queries.bindingIssueSet.IssueID != issueID {
		t.Fatalf("binding issue attachment mismatch: calls=%d arg=%+v", queries.bindingIssueSetCalled, queries.bindingIssueSet)
	}
	assertSenderOpenIDMetadata(t, queries, issueID, "ou_user_a")
	if len(queries.commentsCreated) != 1 {
		t.Fatalf("expected user message mirrored as one comment, got %+v", queries.commentsCreated)
	}
	comment := queries.commentsCreated[0]
	if comment.IssueID != issueID || comment.AuthorType != "member" || comment.AuthorID != sender.MulticaUserID || comment.Content != "check the dashboard" {
		t.Fatalf("mirrored comment mismatch: %+v", comment)
	}
	if res.IssueID != issueID {
		t.Fatalf("dispatch result should carry auto issue id: %+v", res)
	}
	if enq.called != 0 {
		t.Fatalf("auto issue flow must not enqueue a chat task; called=%d", enq.called)
	}
	if enq.calledIssueChat != 1 || enq.issueChatIssue.ID != issueID || enq.issueChatSession != sessionID {
		t.Fatalf("auto issue flow should enqueue one issue-linked chat task; calls=%d issue=%+v session=%+v", enq.calledIssueChat, enq.issueChatIssue, enq.issueChatSession)
	}
	if chat.lastEnsureParams.ThreadRootID != "msg-root" {
		t.Fatalf("root group message should key binding by message id, got %q", chat.lastEnsureParams.ThreadRootID)
	}
}

func TestDispatcher_GroupAutoIssueStoresUnboundSenderOpenID(t *testing.T) {
	sessionID := validUUID(0x66)
	bindingID := validUUID(0x67)
	issueID := validUUID(0x88)
	inst := activeInstallation()
	...
	res, err := d.Handle(context.Background(), InboundMessage{
		...
		AddressedToBot: true,
		ChatID:         "oc_group",
		SenderOpenID:   "ou_unbound_sender",
		Body:           "unbound sender task",
		CommandBody:    "unbound sender task",
		MessageID:      "msg-unbound-root",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if issueSvc.params.CreatorID != inst.InstallerUserID {
		t.Fatalf("unbound creator_id should preserve current member fallback; got %+v want %+v",
			issueSvc.params.CreatorID, inst.InstallerUserID)
	}
	assertSenderOpenIDMetadata(t, queries, issueID, "ou_unbound_sender")
}

func TestAutoIssueContentTruncatesGeneratedTitleOnRuneBoundary(t *testing.T) {
	title, _ := autoIssueContent(InboundMessage{
		CommandBody: "在 https://bytedance.sg.larkoffice.com/docx/ZdnkdkLZIo25APxXgbdlfZFEgFb 的基础上，探查各行业在场域（直播、商城、视频）及招选、补贴视角的数据，重点关注「Fashion、Home & Living、Electronics」三个行业。目的是找到表现不佳的行业及其下钻归因结论，用于指导运营动作。\n请使用 /campaign_concession_temp_probe 技能逐个回溯或二次查询数据",
		CreateTime:  "1765447200000",
	})

	if !utf8.ValidString(title) {
		t.Fatalf("auto issue title must remain valid UTF-8, got %q", title)
	}
	if strings.ContainsRune(title, utf8.RuneError) {
		t.Fatalf("auto issue title must not contain replacement runes, got %q", title)
	}
	if got := utf8.RuneCountInString(title); got > maxAutoIssueTitleRunes {
		t.Fatalf("auto issue title rune count = %d, want <= %d: %q", got, maxAutoIssueTitleRunes, title)
	}
}

func TestDispatcher_GroupAutoIssueAllowsDuplicateGeneratedTitles(t *testing.T) {
	sessionID := validUUID(0x66)
	inst := activeInstallation()
	projectID := validUUID(0x99)
	queries := &fakeQueries{
	...
	created := 0
	issueSvc := &fakeIssueCreator{
		create: func(_ context.Context, p service.IssueCreateParams, _ service.IssueCreateOpts) (service.IssueCreateResult, error) {
			if seen[p.Title] && !p.AllowDuplicate {
				return service.IssueCreateResult{}, service.ErrActiveDuplicate
			}
			seen[p.Title] = true
			created++
			return service.IssueCreateResult{Issue: db.Issue{
				ID:     validUUID(0x80 + byte(created)),
				Number: int32(created),
				Title:  p.Title,
			}}, nil
		},
	}
	...
	for _, messageID := range []string{"msg-same-1", "msg-same-2"} {
		_, err := d.Handle(context.Background(), InboundMessage{
			AppID:          "ok",
			ChatType:       ChatTypeGroup,
			AddressedToBot: true,
			...
			SenderOpenID:   "ou_user_a",
			Body:           "same task",
			CommandBody:    "same task",
			MessageID:      messageID,
		})
		if err != nil {
			t.Fatalf("same generated task title should be accepted for %s: %v", messageID, err)
		}
	}

	if issueSvc.called != 2 {
		t.Fatalf("expected two auto issue create calls, got %d", issueSvc.called)
	}
	if len(issueSvc.paramsHistory) != 2 {
		t.Fatalf("expected two captured create params, got %+v", issueSvc.paramsHistory)
	}
	firstTitle := issueSvc.paramsHistory[0].Title
	if firstTitle != issueSvc.paramsHistory[1].Title {
		t.Fatalf("test setup should exercise a duplicate generated title, got %q and %q", firstTitle, issueSvc.paramsHistory[1].Title)
	}
	if firstTitle != "same task" {
		t.Fatalf("generated auto issue title should not add a disambiguating suffix, got %q", firstTitle)
	}
	for _, params := range issueSvc.paramsHistory {
		if !params.AllowDuplicate {
			t.Fatalf("Lark auto issues must allow duplicate generated titles: %+v", params)
		}
	}
}

func TestDispatcher_GroupIssueCommandUsesTitleOverrideWithoutSecondIssue(t *testing.T) {
	sessionID := validUUID(0x66)
	issueID := validUUID(0x88)
	inst := activeInstallation()
	queries := &fakeQueries{
		...
		userBinding:       boundUser(),
		chatSession:       db.ChatSession{ID: sessionID, AgentID: inst.AgentID},
		agent:             db.Agent{ID: inst.AgentID, LarkIssueProjectID: validUUID(0x99)},
	}
	chat := &fakeChat{
		ensureResult: EnsureChatSessionResult{
			ChatSessionID: sessionID,
		...
		appendResult: AppendResult{IssueCommand: &IssueCommand{Title: "override title", Description: "extra detail"}},
	}
	issueSvc := &fakeIssueCreator{result: service.IssueCreateResult{Issue: db.Issue{ID: issueID, Number: 22, Title: "override title"}}}
	d := &Dispatcher{
		Queries:      queries,
		Chat:         chat,
		Audit:        &fakeAudit{},
	...
	res, err := d.Handle(context.Background(), InboundMessage{
		...
		ChatID:         "oc_group",
		SenderOpenID:   "ou_user_a",
		Body:           "/issue override title\nextra detail",
		CommandBody:    "/issue override title\nextra detail",
		MessageID:      "msg-issue",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	if issueSvc.called != 1 {
		t.Fatalf("/issue in auto-issue flow must not create a second issue, calls=%d", issueSvc.called)
	}
	if issueSvc.params.Title != "override title" || issueSvc.params.Description.String != "extra detail" {
		t.Fatalf("/issue title override not used for auto issue: %+v", issueSvc.params)
	}
	if !issueSvc.params.AllowDuplicate {
		t.Fatalf("Lark group /issue overrides must allow duplicate titles: %+v", issueSvc.params)
	}
	if res.IssueID != issueID {
		t.Fatalf("dispatch result should carry auto issue id: %+v", res)
	}
...
// filter) finalize their claim as processed, so a reconnect replay
// does NOT re-write the audit row. This is the "no audit spam"
// invariant from §4.3.
func TestDispatcher_DropMarksClaim(t *testing.T) {
	...
	_, _ = d.Handle(context.Background(), InboundMessage{
		...
		AddressedToBot: false,
		SenderOpenID:   "ou_user_a",
		MessageID:      "msg-not-addressed",
	})
	if queries.calledMark != 1 {
		t.Fatalf("group-not-addressed drop must mark claim processed; calledMark=%d", queries.calledMark)
	}
	if queries.calledRelease != 0 {
		t.Fatalf("group-not-addressed drop must not release; calledRelease=%d", queries.calledRelease)
	}
}
