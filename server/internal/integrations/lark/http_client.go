...
func (c *httpAPIClient) withTenantTokenRetry(ctx context.Context, creds InstallationCredentials, op string, fn func(token string) error) error {
	...
	token, refreshErr := c.tenantAccessToken(ctx, creds)
	if refreshErr != nil {
		return fmt.Errorf("%w; refresh tenant_access_token for retry failed: %v", err, refreshErr)
	}
	if retryErr := fn(token); retryErr != nil {
		return fmt.Errorf("%s retry after token refresh: %w", op, retryErr)
	}
	return nil
}
...
func (c *httpAPIClient) SendInteractiveCard(ctx context.Context, p SendCardParams) (string, error) {
	...
	if p.ReplyToMessageID == "" && p.ChatID == "" {
	...
	method, path, body := buildSendOrReplyRequest("interactive", p.ChatID, p.CardJSON, p.ReplyToMessageID, p.ReplyInThread)
	if err := c.doJSON(ctx, c.resolveBaseURL(p.InstallationID), method, path, token, body, &resp); err != nil {
...
func (c *httpAPIClient) SendTextMessage(ctx context.Context, p SendTextParams) (string, error) {
	...
	method, path, body := buildSendOrReplyRequest("text", p.ChatID, string(contentBytes), p.ReplyToMessageID, p.ReplyInThread)
	if err := c.doJSON(ctx, c.resolveBaseURL(p.InstallationID), method, path, token, body, &resp); err != nil {
...
func (c *httpAPIClient) SendMarkdownCard(ctx context.Context, p SendMarkdownCardParams) (string, error) {
	...
	method, path, body := buildSendOrReplyRequest("interactive", p.ChatID, string(cardBytes), p.ReplyToMessageID, p.ReplyInThread)
	if err := c.doJSON(ctx, c.resolveBaseURL(p.InstallationID), method, path, token, body, &resp); err != nil {
...
// SendMessageReaction adds an emoji reaction to a specific message via
// POST /open-apis/im/v1/messages/{message_id}/reactions. The reaction
// is attached to the existing message (no new message is created), so
// it serves as a lightweight receipt signal without adding a new bubble.
//
// Returns the reaction_id Lark assigns so the caller can subsequently
// retract the reaction via DeleteMessageReaction.
//
// Best-effort by design: a transient Lark outage or an invalid emoji
// type MUST NOT fail the inbound pipeline. Errors are logged and
// swallowed; the user simply does not see the reaction.
func (c *httpAPIClient) SendMessageReaction(ctx context.Context, p SendReactionParams) (string, error) {
	if p.MessageID == "" {
	...
	err := c.withTenantTokenRetry(ctx, p.InstallationID, "send message reaction", func(token string) error {
		var resp struct {
			Code int    `json:"code"`
			Msg  string `json:"msg"`
			Data struct {
		...
		if err := c.doJSON(ctx, c.resolveBaseURL(p.InstallationID), http.MethodPost, path, token, body, &resp); err != nil {
			return fmt.Errorf("lark http client: send message reaction: %w", err)
		}
		if resp.Code != 0 {
			return larkAPIError{Op: "send message reaction", Code: resp.Code, Msg: resp.Msg}
		}
...
// DeleteMessageReaction retracts a previously-added reaction via
// DELETE /open-apis/im/v1/messages/{message_id}/reactions/{reaction_id}.
// Best-effort: errors are swallowed by the caller; the worst case is a
// stale reaction that never blocks the agent's actual reply.
...
// doMultipart is the multipart/form-data sibling of doJSON: it streams a
// (potentially large) file payload into the request body without
// buffering the bytes in memory. fields are written first as plain form
// values; fileField + filename + body are then added as a single file
// part. body is read exactly once; the caller MUST NOT reuse the same
// io.Reader across retries.
//
// Streaming is set up with io.Pipe — the multipart.Writer runs in a
// goroutine that closes the pipe when the writer's Close flushes the
// trailing boundary. Any error encountered while building the body is
// surfaced by the corresponding read on c.cfg.HTTPClient.Do via
// CloseWithError, so the HTTP layer fails the request rather than
// hanging on a half-written body.
func (c *httpAPIClient) doMultipart(ctx context.Context, baseURL, method, path, token string, fields map[string]string, fileField, filename string, body io.Reader, out any) error {
	pr, pw := io.Pipe()
	...
	go func() {
		var werr error
		defer func() {
			// Close the multipart writer first so the trailing
			// boundary is flushed into the pipe BEFORE we close it.
			// CloseWithError(nil) is equivalent to Close(), which is
			// what the happy path wants.
	...
	req, err := http.NewRequestWithContext(ctx, method, baseURL+path, pr)
	if err != nil {
		return fmt.Errorf("new request: %w", err)
	}
...
// UploadImage uploads an image to Lark via POST /open-apis/im/v1/images
// and returns the resulting `image_key`. The body is streamed through
// io.Pipe + multipart.Writer so a large image is never fully buffered
// in memory. Token-expiry errors (99991663 / 99991664) drop the cached
// tenant_access_token; the caller's next attempt refreshes naturally —
// this matches the rest of this client's outbound paths (no internal
// retry loop, so a single misbehaving install can't burn duplicate
// uploads).
func (c *httpAPIClient) UploadImage(ctx context.Context, creds InstallationCredentials, p UploadImageParams) (string, error) {
	if p.Reader == nil {
		return "", errors.New("lark http client: missing image reader")
	}
	if p.ImageType == "" {
		return "", errors.New("lark http client: missing image_type")
	}
	token, err := c.tenantAccessToken(ctx, creds)
	if err != nil {
		return "", err
	...
	fields := map[string]string{"image_type": p.ImageType}
	var resp struct {
		Code int    `json:"code"`
		Msg  string `json:"msg"`
		Data struct {
	...
	if err := c.doMultipart(ctx, c.resolveBaseURL(creds), http.MethodPost, "/open-apis/im/v1/images", token, fields, "image", filename, p.Reader, &resp); err != nil {
	...
	if resp.Code != 0 || resp.Data.ImageKey == "" {
		...
		return "", fmt.Errorf("lark http client: upload image: code=%d msg=%q", resp.Code, resp.Msg)
	}
	return resp.Data.ImageKey, nil
}

// buildSendOrReplyRequest selects between Lark's IM v1 send endpoint
// (POST /open-apis/im/v1/messages?receive_id_type=chat_id) and the
// per-message reply endpoint (POST /open-apis/im/v1/messages/{message_id}/reply)
// based on whether replyToMessageID is set. Used by every outbound that
// can run as either a fresh send or a thread reply — group-chat replies
// are routed to the reply path with replyInThread=true so multiple
// coworkers @-mentioning the bot in parallel each see their own
// conversation grouped under their prompt instead of interleaved in
// the main timeline.
func buildSendOrReplyRequest(msgType string, chatID ChatID, content, replyToMessageID string, replyInThread bool) (method, path string, body map[string]any) {
	if replyToMessageID != "" {
		body = map[string]any{
			"msg_type": msgType,
			"content":  content,
		...
		return http.MethodPost,
			"/open-apis/im/v1/messages/" + url.PathEscape(replyToMessageID) + "/reply",
			body
	}
	q := url.Values{}
