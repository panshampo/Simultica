...
type ChatTaskEnqueuer interface {
	...
	EnqueueTaskForIssueChat(ctx context.Context, issue db.Issue, chatSessionID pgtype.UUID) (db.AgentTaskQueue, error)
}
...
type DispatcherQueries interface {
	...
	// GetAgent reads agent.lark_issue_project_id so the dispatcher can
	// decide whether to auto-create an Issue on the first @-mention of
	// a thread. Lookup failure degrades to chat-only behavior — we do
	// not abort dispatch on it.
	GetAgent(ctx context.Context, id pgtype.UUID) (db.Agent, error)
	// GetIssue loads an existing auto-created issue for follow-up
	// messages in a Lark thread. The binding row only stores issue_id;
	// scheduling the canonical issue-backed chat task needs the full row.
	GetIssue(ctx context.Context, id pgtype.UUID) (db.Issue, error)
	// SetLarkChatSessionBindingIssue attaches the auto-created issue to
	// the binding row. Idempotency is enforced by the query (issue_id
	// IS NULL guard), so a race or replay cannot overwrite a prior
	// attachment.
	SetLarkChatSessionBindingIssue(ctx context.Context, arg db.SetLarkChatSessionBindingIssueParams) error
	// CreateComment mirrors a Lark message into the auto-created
	// issue's comment thread. Author type is "agent" or "member" per
	// the issue.creator_type contract.
	CreateComment(ctx context.Context, arg db.CreateCommentParams) (db.Comment, error)
	// SetIssueMetadataKey stores transport-native provenance on the
	// auto-created issue without changing the issue.creator_id contract,
	// which must remain a Multica member UUID.
...
func (d *Dispatcher) processClaimed(ctx context.Context, msg InboundMessage, inst db.LarkInstallation, claimToken pgtype.UUID) (DispatchResult, dedupFinalize, error) {
	...
	// 4. Resolve sender identity. If the open_id maps to a bound
	//    workspace member, credit them; otherwise fall back to the
	//    installer so unbound coworkers can still talk to the bot.
	senderUserID := inst.InstallerUserID
	...
	if err == nil {
		senderUserID = binding.MulticaUserID
	}
	...
	//    so we use them.
	//
	//    Thread keying: a single Lark group chat can host many
	//    parallel @-mention threads. We key the binding on
	//    (installation, lark_chat_id, thread_root_id) so each thread
	//    gets its own chat_session — and, when the agent has
	//    lark_issue_project_id set, its own auto-created Issue. p2p
	//    has no thread concept, so it always uses the empty string.
	sessionCreator := senderUserID
	if msg.ChatType == ChatTypeGroup {
		sessionCreator = inst.InstallerUserID
	}
	threadRootID := threadRootForMessage(msg)
	ensureRes, err := d.Chat.EnsureChatSession(ctx, EnsureChatSessionParams{
	...
	sessionID := ensureRes.ChatSessionID

	// 5a. Auto-create the per-thread Issue on first contact. Only
	//     fires when this dispatch CREATED the binding (subsequent
	//     messages in the thread reuse the existing binding) AND the
	//     agent has lark_issue_project_id set. Failures are
	//     warn-logged and the dispatch continues — the chat_session is
	//     already durable; an issue-create blip should not eat the
	//     user's message.
	if ensureRes.Created && ensureRes.BindingID.Valid {
		if issue, ok := d.maybeAutoCreateIssue(ctx, inst, senderUserID, sessionID, ensureRes.BindingID, msg); ok {
			ensureRes.IssueID = issue.ID
			ensureRes.Issue = issue
		}
	}
	if ensureRes.IssueID.Valid && !ensureRes.Issue.ID.Valid {
		issue, err := d.Queries.GetIssue(ctx, ensureRes.IssueID)
		if err != nil {
			return DispatchResult{}, finalizeRelease, fmt.Errorf("load auto issue: %w", err)
		}
		ensureRes.Issue = issue
	}
	...
	// Carry the auto-issue id forward so callers / tests can observe it
	// even when no /issue command was given on this message.
	if ensureRes.IssueID.Valid {
		res.IssueID = ensureRes.IssueID
		res.Issue = ensureRes.Issue
	}

	// Mirror the user's message into the auto-issue's comment thread so
	// the issue page shows the same conversation as Lark. Best-effort:
	// chat_message is already durable, and a comment write blip cannot
	// roll that back. The Patcher does the same for agent replies on
	// the outbound side.
	if ensureRes.IssueID.Valid && msg.ChatType == ChatTypeGroup {
		d.mirrorUserMessageToIssue(ctx, ensureRes.IssueID, inst.WorkspaceID, senderUserID, msg.Body)
	}
	...
	if appendRes.IssueCommand != nil {
		if ensureRes.IssueID.Valid {
			// Group auto-issue already created the thread issue before
			// append. `/issue` in this flow only overrides the auto-title;
			// it must not create a second Issue for the same Lark thread.
			res.IssueID = ensureRes.IssueID
			res.IssueTitle = appendRes.IssueCommand.Title
		} else {
			issueRes, err := d.createIssueFromCommand(ctx, inst, senderUserID, sessionID, *appendRes.IssueCommand, msg.SenderOpenID)
			if err != nil {
	...
	if ensureRes.IssueID.Valid && msg.ChatType == ChatTypeGroup && ensureRes.Issue.ID.Valid {
		d.scheduleIssueRun(inst, msg, sessionID, ensureRes.Issue)
	} else {
...
func (d *Dispatcher) scheduleIssueRun(inst db.LarkInstallation, msg InboundMessage, sessionID pgtype.UUID, issue db.Issue) {
	flush := func() { d.flushIssueRun(inst, msg, sessionID, issue) }
	if d.batcher == nil {
		flush()
		return
	}
...
func (d *Dispatcher) flushIssueRun(inst db.LarkInstallation, msg InboundMessage, sessionID pgtype.UUID, issue db.Issue) {
	ctx, cancel := context.WithTimeout(context.Background(), chatRunFlushTimeout)
	defer cancel()

	if _, err := d.TaskService.EnqueueTaskForIssueChat(ctx, issue, sessionID); err != nil {
		switch {
		case errors.Is(err, service.ErrChatTaskAgentNoRuntime):
			d.emitFlushReply(ctx, inst, msg, sessionID, OutcomeAgentOffline)
		case errors.Is(err, service.ErrChatTaskAgentArchived):
			d.emitFlushReply(ctx, inst, msg, sessionID, OutcomeAgentArchived)
		default:
			d.logger().Error("lark dispatcher: flush enqueue issue chat task failed",
...
func (d *Dispatcher) createIssueFromCommand(
	...
	res, err := d.IssueService.Create(ctx, params, service.IssueCreateOpts{})
	if err != nil {
		return res, err
	}
	res.Issue = d.storeIssueLarkSenderOpenID(ctx, res.Issue, senderOpenID)
	return res, nil
}

// threadRootForMessage picks the binding key that distinguishes one
// Lark thread from another within the same chat. The Lark RootID
// already points at the thread anchor for replies; for the very first
// message in a new thread (no RootID) we fall back to MessageID. p2p
// chats have no thread concept, so they always collapse to the empty
// string.
func threadRootForMessage(msg InboundMessage) string {
	if msg.ChatType != ChatTypeGroup {
		return ""
	}
	if msg.RootID != "" {
...
// maybeAutoCreateIssue creates the per-thread Issue when the agent has
// lark_issue_project_id set and attaches it to the binding row. The
// chat_message has not yet been written when this runs, so the Issue's
// title is derived from msg.CommandBody (the user's typed text), with
// fallbacks to Body and a default "Lark thread" label. Failures are
// warn-logged and the dispatch continues with chat-only behavior — the
// user's message is more important than the auto-issue mirror.
func (d *Dispatcher) maybeAutoCreateIssue(
	ctx context.Context,
	inst db.LarkInstallation,
	...
	agent, err := d.Queries.GetAgent(ctx, inst.AgentID)
	if err != nil {
		d.logger().Warn("lark dispatcher: load agent for auto-issue failed",
			"agent_id", uuidString(inst.AgentID),
			"err", err.Error(),
		)
		return db.Issue{}, false
	}
	if !agent.LarkIssueProjectID.Valid {
		return db.Issue{}, false
	}
	title, description := autoIssueContent(msg)
	res, err := d.IssueService.Create(ctx, service.IssueCreateParams{
		WorkspaceID:    inst.WorkspaceID,
		Title:          title,
		...
		ProjectID:      agent.LarkIssueProjectID,
		OriginType:     pgtype.Text{String: originLarkChat, Valid: true},
		OriginID:       sessionID,
		AllowDuplicate: true,
	}, service.IssueCreateOpts{SkipEnqueueOnAssign: true})
	if err != nil {
		d.logger().Warn("lark dispatcher: auto-issue create failed",
			"agent_id", uuidString(inst.AgentID),
			"project_id", uuidString(agent.LarkIssueProjectID),
			"err", err.Error(),
		)
		return db.Issue{}, false
	}
	res.Issue = d.storeIssueLarkSenderOpenID(ctx, res.Issue, msg.SenderOpenID)
	if err := d.Queries.SetLarkChatSessionBindingIssue(ctx, db.SetLarkChatSessionBindingIssueParams{
		ID:      bindingID,
		IssueID: res.Issue.ID,
	}); err != nil {
		d.logger().Warn("lark dispatcher: attach auto-issue to binding failed",
			"binding_id", uuidString(bindingID),
			"issue_id", uuidString(res.Issue.ID),
			"err", err.Error(),
		)
		// The Issue row exists; the binding update missed. Return
		// the id anyway so this turn's mirror can still go through.
	}
	return res.Issue, true
}

const metadataLarkSenderOpenID = "lark_sender_open_id"

func (d *Dispatcher) storeIssueLarkSenderOpenID(ctx context.Context, issue db.Issue, sender OpenID) db.Issue {
	if !issue.ID.Valid || sender == "" {
		return issue
	}
	value, err := json.Marshal(string(sender))
	if err != nil {
		return issue
	}
	_, err = d.Queries.SetIssueMetadataKey(ctx, db.SetIssueMetadataKeyParams{
		ID:          issue.ID,
		WorkspaceID: issue.WorkspaceID,
		Key:         metadataLarkSenderOpenID,
		Value:       value,
	})
	if err != nil {
		d.logger().Warn("lark dispatcher: store issue sender open_id failed",
			"issue_id", uuidString(issue.ID),
			"err", err.Error(),
		)
		return issue
...
// autoIssueContent derives a concise title and optional description for
// the auto-created per-thread Issue. `/issue ...` is treated as a title
// override escape hatch rather than a request for a second issue.
const maxAutoIssueTitleRunes = 120

func autoIssueContent(msg InboundMessage) (string, pgtype.Text) {
	commandSource := msg.CommandBody
	if commandSource == "" {
		commandSource = msg.Body
	}
	if cmd, ok := parseIssueCommand(commandSource); ok {
		if strings.TrimSpace(cmd.Title) != "" {
			return cmd.Title, pgtype.Text{String: cmd.Description, Valid: cmd.Description != ""}
		}
	}
	...
	pick := func(s string) string {
		for _, line := range strings.Split(s, "\n") {
			if t := strings.TrimSpace(line); t != "" {
				return truncateRunes(t, maxAutoIssueTitleRunes)
			}
		}
		return ""
	}
	if t := pick(msg.CommandBody); t != "" {
		return t, pgtype.Text{}
	}
	if t := pick(msg.Body); t != "" {
		return t, pgtype.Text{}
	}
	return "Lark thread", pgtype.Text{}
}
...
// mirrorUserMessageToIssue posts the user's Lark message as a comment
// on the auto-created issue so the issue page shows the same
// conversation. Best-effort: a failed mirror does not roll back the
// chat_message that was already committed.
func (d *Dispatcher) mirrorUserMessageToIssue(
	ctx context.Context,
	issueID pgtype.UUID,
	...
	if _, err := d.Queries.CreateComment(ctx, db.CreateCommentParams{
		IssueID:     issueID,
		...
		d.logger().Warn("lark dispatcher: mirror user comment failed",
