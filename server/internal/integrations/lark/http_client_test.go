...
func TestHTTPClient_SendMessageReaction_TokenExpired_RetriesOnce(t *testing.T) {
	fake := newLarkFake(t)
	fake.stubToken("tok_reaction", 7200)
	...
	fake.mux.HandleFunc("/open-apis/im/v1/messages/om_1/reactions", func(w http.ResponseWriter, r *http.Request) {
		n := reactionCalls.Add(1)
		if r.Method != http.MethodPost {
			t.Errorf("reaction: want POST, got %s", r.Method)
		}
		switch n {
		case 1:
			if got := r.Header.Get("Authorization"); got != "Bearer tok_reaction" {
				t.Errorf("first reaction Authorization = %q", got)
			}
			writeJSON(w, map[string]any{"code": codeTokenExpired, "msg": "expired"})
		case 2:
			if got := r.Header.Get("Authorization"); got != "Bearer tok_reaction" {
				t.Errorf("retry reaction Authorization = %q", got)
			}
			writeJSON(w, map[string]any{
				"code": 0,
				"data": map[string]string{"reaction_id": "reaction-after-refresh"},
			})
		default:
			t.Fatalf("unexpected reaction call #%d", n)
		}
	})

	c := newTestClient(fake, time.Now)
	reactionID, err := c.SendMessageReaction(context.Background(), SendReactionParams{
		InstallationID: testCreds(),
		MessageID:      "om_1",
		EmojiType:      "OneSecond",
	})
	if err != nil {
		t.Fatalf("SendMessageReaction: %v", err)
	}
	if reactionID != "reaction-after-refresh" {
		t.Errorf("reactionID = %q", reactionID)
	}
	if got := fake.tokenN.Load(); got != 2 {
		t.Errorf("token endpoint hits = %d, want 2", got)
	}
	if got := reactionCalls.Load(); got != 2 {
		t.Errorf("reaction endpoint hits = %d, want 2", got)
	}
}
