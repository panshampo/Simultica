...
type APIClient interface {
	...
	// SendMessageReaction adds an emoji reaction to an existing Lark
	// message. Used for lightweight "message received" acknowledgment
	// without adding noise to the conversation — a thumbs-up on the
	// user's own message is visually clear but unobtrusive. Returns the
	// reaction_id Lark assigns so the caller can later delete it via
	// DeleteMessageReaction once the agent's real reply lands.
	SendMessageReaction(ctx context.Context, p SendReactionParams) (string, error)

	// DeleteMessageReaction removes a previously-added reaction. The
	// Patcher calls this once the agent's actual reply has been delivered
	// so the receipt-style reaction is retracted instead of sticking
	// around as visual noise alongside the answer.
	...
	// UploadImage uploads an image to Lark via
	// POST /open-apis/im/v1/images and returns the resulting `image_key`.
	// The body is streamed with multipart/form-data so a large image is
	// never fully buffered in memory. Token-expiry errors (99991663 /
	// 99991664) drop the cached tenant_access_token so a caller-driven
	// retry refreshes naturally — matching the rest of this client's
	// outbound paths.
	UploadImage(ctx context.Context, creds InstallationCredentials, p UploadImageParams) (string, error)
}

// UploadImageParams is the input shape for UploadImage.
//
// Reader is consumed exactly once and streamed into the multipart body
// via io.Pipe + io.Copy; callers MUST NOT pre-buffer the bytes into
// memory.
//
// ImageType is the Lark image_type form field — "message" for normal
// chat / card content, "avatar" for bot/user avatars. Filename is the
// form-data filename Lark records; it does not affect the resulting
// image_key but is surfaced in Lark error messages, so a meaningful
// value (e.g. "screenshot.png") aids debugging.
...
type SendCardParams struct {
	...
	CardJSON string
	// ReplyToMessageID, when non-empty, routes the send through Lark's
	// /open-apis/im/v1/messages/{message_id}/reply endpoint so the
	// outbound shows up as a reply (or, with ReplyInThread=true, as a
	// thread reply) of the referenced message. Used to keep group-chat
	// replies grouped when multiple coworkers @-mention the bot in
	// parallel. ChatID is ignored on the reply path; Lark derives it
...
type SendTextParams struct {
	...
	ChatID         ChatID
	Text           string
	// ReplyToMessageID / ReplyInThread mirror SendCardParams: when
	// ReplyToMessageID is non-empty the send is routed through the
	// reply endpoint so it lands as a reply (or thread reply) under
	// the referenced message instead of as a fresh chat message.
	ReplyToMessageID string
	ReplyInThread    bool
}
...
type SendMarkdownCardParams struct {
	...
	Summary string
	// ReplyToMessageID / ReplyInThread mirror SendCardParams.
	ReplyToMessageID string
	ReplyInThread    bool
}
...
// SendReactionParams carries the data needed to add an emoji reaction to
// a specific Lark message. The reaction is attached to the message itself
// (not posted as a new message), so it serves as a lightweight receipt
// signal without adding a new bubble to the conversation.
type SendReactionParams struct {
	InstallationID InstallationCredentials
	MessageID      string
	// EmojiType is the Lark emoji type key, e.g. "THUMBSUP", "OK",
	// "DONE". The full list is in Lark's open-platform docs; invalid
	// types are silently ignored by Lark (no error, the reaction just
	// does not appear). The caller is responsible for picking a type
	// that exists in the target Lark tenant.
	EmojiType string
}

// DeleteReactionParams carries the data needed to remove a previously-
// added reaction. ReactionID is the id Lark returned from
// SendMessageReaction; the caller is responsible for tracking it across
// the receipt → reply lifecycle.
...
func (s *stubAPIClient) SendMessageReaction(ctx context.Context, p SendReactionParams) (string, error) {
	s.log.Warn("lark stub client: SendMessageReaction called", "message_id", p.MessageID, "emoji", p.EmojiType)
	return "", ErrAPIClientNotConfigured
}
...
func (s *stubAPIClient) UploadImage(ctx context.Context, creds InstallationCredentials, p UploadImageParams) (string, error) {
	s.log.Warn("lark stub client: UploadImage called", "image_type", p.ImageType, "filename", p.Filename)
	return "", ErrAPIClientNotConfigured
}
