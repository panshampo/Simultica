...
func NewLarkOutcomeReplier(cfg OutcomeReplierConfig) OutcomeReplier {
	...
	return &LarkOutcomeReplier{
		...
		queries:       cfg.Queries,
		tracker:       cfg.Tracker,
		threadAnchors: cfg.ThreadAnchors,
...
func (r *LarkOutcomeReplier) Reply(ctx context.Context, inst db.LarkInstallation, msg InboundMessage, res DispatchResult) {
	// Record the trigger message_id as the thread anchor for this
	// chat_session before dispatching any reply. The Patcher and the
	// synchronous notices below both consult the anchor so every
	// outbound for this session lands as a thread-reply under the
	// user's @-mention. Group chats only — p2p has no interleaving
	// problem and threading just adds visual noise.
	if r.threadAnchors != nil && msg.ChatType == ChatTypeGroup && msg.MessageID != "" && res.ChatSessionID.Valid {
		r.threadAnchors.Set(res.ChatSessionID, msg.MessageID)
	}
	switch res.Outcome {
	...
	case OutcomeIngested:
		...
		// command itself was understood. Auto-created per-thread Issues
		// do not need this extra "Created ..." message — the bot's real
		// answer arrives shortly after, and surfacing a synthetic create
		// notice in that flow just adds noise. Gate on IssueIdentifier /
		// IssueNumber so only the explicit `/issue` path sends this.
		if res.IssueID.Valid && (res.IssueIdentifier != "" || res.IssueNumber > 0) {
		...
		// Add a thumbs-up reaction to the user's message so they know
		// the bot received it. This is a lightweight receipt signal
		// that does not add a new message bubble to the conversation.
		// Best-effort: a transient Lark error here must not fail the
		// inbound pipeline.
		if msg.MessageID != "" {
			if err := r.sendReaction(ctx, inst, msg.MessageID, res.ChatSessionID); err != nil {
				r.log.Warn("lark outcome replier: reaction failed",
					"installation_id", uuidString(inst.ID),
...
// sendReaction adds a 👍 emoji reaction to the user's message so they
// know the bot received it. Best-effort: errors are logged and
// swallowed so a transient Lark outage cannot break the inbound
// pipeline. When a tracker is wired, the returned reaction_id is
// recorded under the chat_session so the Patcher can retract the
// reaction once the agent's reply for the same session lands.
func (r *LarkOutcomeReplier) sendReaction(ctx context.Context, inst db.LarkInstallation, messageID string, chatSessionID pgtype.UUID) error {
	if messageID == "" {
	...
	reactionID, err := r.client.SendMessageReaction(ctx, SendReactionParams{
	...
	if r.tracker != nil && reactionID != "" {
		r.tracker.Track(chatSessionID, messageID, reactionID)
	}
	return nil
}
...
func (r *LarkOutcomeReplier) sendIssueCreated(ctx context.Context, inst db.LarkInstallation, msg InboundMessage, res DispatchResult) error {
	...
	anchor, inThread := r.replyAnchor(msg, res.ChatSessionID)
...
func (r *LarkOutcomeReplier) sendChatNotice(ctx context.Context, inst db.LarkInstallation, msg InboundMessage, body string) error {
	...
	anchor, inThread := r.replyAnchor(msg, pgtype.UUID{})
...
// replyAnchor decides whether this synchronous outbound should be
// routed through Lark's reply-in-thread endpoint. Group-chat triggers
// always thread under their @-mention so concurrent users do not see
// interleaved replies; the chat_session-keyed tracker is consulted
// when a session id is available, otherwise (offline/archived notices
// without a persisted session) we fall back to the inbound message_id
// directly. p2p messages bypass this entirely.
func (r *LarkOutcomeReplier) replyAnchor(msg InboundMessage, chatSessionID pgtype.UUID) (string, bool) {
	if msg.ChatType != ChatTypeGroup {
		return "", false
	}
	if r.threadAnchors != nil && chatSessionID.Valid {
		if id := r.threadAnchors.Get(chatSessionID); id != "" {
			return id, true
		}
	}
	if msg.MessageID != "" {
