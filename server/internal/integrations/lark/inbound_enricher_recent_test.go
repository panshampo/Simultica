...
func TestEnrichRecentContextDisabledForAddressedGroupMessage(t *testing.T) {
	t.Parallel()
	fake := newEnricherFake()
	fake.byChat["oc_g"] = []LarkMessage{
		textMsg("om_prev", "ou_alice", "上一条消息", "1000"),
		textMsg("om_trigger", "ou_user", "只看当前输入", "2000"),
	}
	in := InboundMessage{
		MessageType:    "text",
		MessageID:      "om_trigger",
	...
	out := enrich(t, fake, in, InboundEnricherConfig{RecentContextSize: 0})

	if out.Body != in.Body {
		t.Fatalf("body = %q, want unchanged %q when recent context is disabled", out.Body, in.Body)
	}
	if len(fake.listCalls) != 0 {
		t.Fatalf("expected no ListChatMessages calls when recent context is disabled, got %v", fake.listCalls)
	}
}
