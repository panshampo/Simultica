...
// Lark chat thread. The implementation is the two-phase find-or-create
// expected by the interface contract:
//
//  1. Look up the existing lark_chat_session_binding by
//     (installation, lark_chat_id, lark_thread_root_id).
//  2. If found, return its chat_session_id (and issue_id, if any).
...
// thread is resolved by the UNIQUE
// (installation_id, lark_chat_id, lark_thread_root_id) constraint on
// lark_chat_session_binding: the loser of the race catches the unique
// violation, re-reads the existing row, and returns its session.
func (s *chatSessionService) EnsureChatSession(ctx context.Context, p EnsureChatSessionParams) (EnsureChatSessionResult, error) {
	...
	existing, err := s.queries.GetLarkChatSessionBinding(ctx, db.GetLarkChatSessionBindingParams{
		...
		LarkThreadRootID: p.ThreadRootID,
	})
	if err == nil {
		return EnsureChatSessionResult{
	...
	if !errors.Is(err, pgx.ErrNoRows) {
		return EnsureChatSessionResult{}, fmt.Errorf("lookup chat session binding: %w", err)
	}

	// Create path: chat_session + binding atomically.
	res, err := s.createSessionAndBinding(ctx, p)
	if err == nil {
		res.Created = true
		return res, nil
	}
	...
	if isUniqueViolation(err) {
		...
		return EnsureChatSessionResult{}, fmt.Errorf("race re-read after unique violation: %w", lookupErr)
	}
	return EnsureChatSessionResult{}, err
}

func (s *chatSessionService) createSessionAndBinding(ctx context.Context, p EnsureChatSessionParams) (EnsureChatSessionResult, error) {
	tx, err := s.txStarter.Begin(ctx)
	if err != nil {
		return EnsureChatSessionResult{}, fmt.Errorf("begin tx: %w", err)
	}
	...
	if err != nil {
		return EnsureChatSessionResult{}, fmt.Errorf("create chat session: %w", err)
	}

	binding, err := qtx.CreateLarkChatSessionBinding(ctx, db.CreateLarkChatSessionBindingParams{
	...
	if err := tx.Commit(ctx); err != nil {
		return EnsureChatSessionResult{}, fmt.Errorf("commit: %w", err)
	}
	return EnsureChatSessionResult{
		ChatSessionID: session.ID,
