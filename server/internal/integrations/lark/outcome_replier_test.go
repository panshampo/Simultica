...
func (s *stubAPIClientWithRecorder) SendMessageReaction(ctx context.Context, p SendReactionParams) (string, error) {
	return "", nil
}
...
func (s *stubAPIClientWithRecorder) UploadImage(ctx context.Context, creds InstallationCredentials, p UploadImageParams) (string, error) {
	return "", nil
}
...
// Auto-created per-thread Issues are a background bookkeeping action for
// group-chat context isolation; unlike the explicit `/issue` command, they
// should not generate a separate "Created M-xx" confirmation bubble in Lark.
func TestLarkOutcomeReplierAutoIssueIsSilent(t *testing.T) {
	t.Parallel()
	log := slog.New(slog.NewTextHandler(io.Discard, nil))
	stub := &stubAPIClientWithRecorder{configured: true}
	rep := NewLarkOutcomeReplier(OutcomeReplierConfig{
	...
	rep.Reply(context.Background(), db.LarkInstallation{AppID: "cli_x"}, InboundMessage{ChatID: "oc"},
		DispatchResult{
			Outcome: OutcomeIngested,
			IssueID: mustUUID("22222222-2222-2222-2222-222222222222"),
		})
	...
	defer stub.mu.Unlock()
	if len(stub.textOut) != 0 || len(stub.interactiveOut) != 0 {
		t.Errorf("auto-created issue must not emit confirmation text; got text=%d cards=%d",
			len(stub.textOut), len(stub.interactiveOut))
	}
}
