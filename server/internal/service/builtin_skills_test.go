...
// TestChartRenderSkillCoversContract pins the skill to the renderer it
// actually documents: the SKILL.md must surface the JSON input contract,
// the supported chart types, the 10MB cap, and the relative-output rule
// (which is the seam between the skill and the runtime brief's Image
// Output section). It must also ship the renderer script and the source
// map so a future drift in chart_render.py shows up as a failing test
// here, not as an agent following a stale doc.
func TestChartRenderSkillCoversContract(t *testing.T) {
	skill, ok := findSkill(t, "multica-chart-render")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	if got := strings.TrimSpace(fm["user-invocable"]); got != "false" {
		t.Errorf("user-invocable = %q, want false (chart rendering triggers from context, not a slash command)", got)
	}
	if got := strings.TrimSpace(fm["allowed-tools"]); !strings.Contains(got, "uv") && !strings.Contains(got, "python3") {
		t.Errorf("allowed-tools = %q, want at least Bash(uv *) or Bash(python3 *) — the skill must fence to the runners it teaches", got)
	}
	...
	mustContain := []string{
		...
		"row_shape_mismatch",
		"path_escape",
		"references/chart-render-source-map.md",
	}
	for _, want := range mustContain {
		if !strings.Contains(body, want) {
			t.Errorf("chart-render skill missing %q", want)
		}
	}

	if !skillHasFile(skill, "scripts/chart_render.py") {
		t.Errorf("chart-render skill missing supporting file scripts/chart_render.py")
	}
	if !skillHasFile(skill, "scripts/requirements.txt") {
		t.Errorf("chart-render skill missing supporting file scripts/requirements.txt")
	}
	if !skillHasFile(skill, "references/chart-render-source-map.md") {
		t.Errorf("chart-render skill missing supporting file references/chart-render-source-map.md")
	}
}
