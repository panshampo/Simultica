...
func (s *TaskService) EnqueueTaskForIssue(ctx context.Context, issue db.Issue, triggerCommentID ...pgtype.UUID) (db.AgentTaskQueue, error) {
	...
	return s.enqueueIssueTask(ctx, issue, commentID, pgtype.UUID{}, false)
}

func (s *TaskService) EnqueueWorkflowPlannerTask(ctx context.Context, issue db.Issue, agentID pgtype.UUID) (db.AgentTaskQueue, error) {
	...
	task, err := s.Queries.CreateWorkflowPlannerTask(ctx, db.CreateWorkflowPlannerTaskParams{
		AgentID:   agentID,
		RuntimeID: agent.RuntimeID,
		IssueID:   issue.ID,
		Priority:  priorityToInt(issue.Priority),
		Context:   []byte(`{"type":"workflow_planner"}`),
	})
	if err != nil {
		return db.AgentTaskQueue{}, fmt.Errorf("create workflow planner task: %w", err)
	}
...
func (s *TaskService) EnqueueWorkflowMainNodeTask(ctx context.Context, issue db.Issue, agentID, workflowRunID pgtype.UUID, nodeID, nodeType, renderedContext string, inputSnapshot []byte) (db.AgentTaskQueue, error) {
	...
	if err != nil {
		return db.AgentTaskQueue{}, fmt.Errorf("create workflow main node task: %w", err)
	}
	s.broadcastTaskEvent(ctx, protocol.EventTaskQueued, task)
	s.NotifyTaskEnqueued(ctx, task)
	return task, nil
...
// EnqueueTaskForIssueChat creates the single canonical task for an
// issue-backed chat thread. The task is issue-linked so completion writes
// the Issue comment, and chat-linked so Lark outbound can reply to the thread.
func (s *TaskService) EnqueueTaskForIssueChat(ctx context.Context, issue db.Issue, chatSessionID pgtype.UUID) (db.AgentTaskQueue, error) {
	return s.enqueueIssueTask(ctx, issue, pgtype.UUID{}, chatSessionID, false)
}
...
// expected behavior.
func (s *TaskService) enqueueIssueTask(ctx context.Context, issue db.Issue, triggerCommentID pgtype.UUID, chatSessionID pgtype.UUID, forceFreshSession bool) (db.AgentTaskQueue, error) {
...
func (s *TaskService) CompleteTask(ctx context.Context, taskID pgtype.UUID, result []byte, sessionID, workDir string) (*db.AgentTaskQueue, error) {
	...
	// Chat-backed tasks publish chat:done so downstream transports (for
	// example the Lark patcher) can deliver the agent's final reply. Only
	// direct chat tasks persist a Multica assistant chat_message; issue-backed
	// Lark threads intentionally skip that storage path so they do not appear
	// in the desktop chat UI.
	if task.ChatSessionID.Valid {
		var assistantMsg *db.ChatMessage
		var payload protocol.TaskCompletedPayload
		if !task.IssueID.Valid && json.Unmarshal(result, &payload) == nil && payload.Output != "" {
...
func (s *TaskService) enqueueRerunTask(ctx context.Context, issue db.Issue, agentID pgtype.UUID, triggerCommentID pgtype.UUID, isLeader bool) (db.AgentTaskQueue, error) {
	if issue.AssigneeType.String == "agent" && issue.AssigneeID.Valid &&
		...
		return s.enqueueIssueTask(ctx, issue, triggerCommentID, pgtype.UUID{}, true)
	}
...
func (s *TaskService) broadcastChatDone(ctx context.Context, task db.AgentTaskQueue, msg *db.ChatMessage) {
	...
	if task.IssueID.Valid {
		var parsed protocol.TaskCompletedPayload
		if err := json.Unmarshal(task.Result, &parsed); err == nil && parsed.Output != "" {
			payload.Content = redact.Text(util.UnescapeBackslashEscapes(parsed.Output))
		}
	}
	if msg != nil {
