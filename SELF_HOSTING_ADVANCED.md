...
| `MULTICA_DS_COPILOT_PATH` | Custom path to the `ds-copilot` executable wrapper or launcher script (for example `~/.ds-copilot/launch.sh`). DS Copilot model selection is managed by its launcher/CCR config, not by a Multica model env var. |
