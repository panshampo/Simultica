 ...
 *   - "69441a2" (no tag)       → "0.0.0-69441a2"         (fallback)
 *
...
export function normalizeGitVersion(raw) {
  ...
  if (isSemverVersion(stripped)) {
    return stripped;
  }
  // No reachable tag, or another non-semver git describe shape. Commit
  // hashes can start with a digit (`69441a2`), so do not infer validity from
  // the first character; electron-updater requires a full semver value.
  return `0.0.0-${sanitizePrereleaseIdentifier(stripped)}`;
}
