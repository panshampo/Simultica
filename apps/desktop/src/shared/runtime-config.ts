...
export type RuntimeConfigSaveResult =
  | { ok: true; config: RuntimeConfig; path: string }
...
export function parseRuntimeConfig(raw: string): RuntimeConfig {
  ...
  return runtimeConfigFromObject(parsed);
}

export function runtimeConfigFromInput(input: RuntimeConfigInput): RuntimeConfig {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw new Error("Invalid desktop runtime config: expected a JSON object");
  }
  return runtimeConfigFromObject({ schemaVersion: 1, ...input });
}

function runtimeConfigFromObject(parsed: unknown): RuntimeConfig {
