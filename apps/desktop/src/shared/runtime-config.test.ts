...
describe("runtime config", () => {
  ...
  it("normalizes writable desktop config from a single server URL", () => {
    expect(runtimeConfigFromInput({ apiUrl: " http://192.0.2.10/ " })).toEqual({
      schemaVersion: 1,
  ...
  it("accepts split self-hosted API and web app URLs", () => {
    expect(
      runtimeConfigFromInput({
