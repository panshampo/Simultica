...
const PREFS_PATH = join(homedir(), ".multicaa", "desktop_prefs.json");
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".multicaa", "profiles", profile)
    : join(homedir(), ".multicaa");
}
...
function cliProfileDir(profile: string): string {
  return profile
    ? join(homedir(), ".multica", "profiles", profile)
    : join(homedir(), ".multica");
}

function cliProfileConfigPath(profile: string): string {
  return join(cliProfileDir(profile), "config.json");
}
...
//   ~/.multicaa/profiles/desktop-<host>/
...
async function readCliProfileConfig(
  profile: string,
): Promise<Record<string, unknown>> {
  try {
    const raw = await readFile(cliProfileConfigPath(profile), "utf-8");
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed === "object" ? parsed : {};
  } catch {
    return {};
...
async function writeProfileConfig(
  ...
  const op = async () => {
    ...
    const existingCliCfg = await readCliProfileConfig(profile);
    const cliCfg = {
      ...existingCliCfg,
      ...cfg,
    };
    if (
      typeof cfg.workspace_id !== "string" &&
      typeof existingCliCfg.workspace_id === "string"
    ) {
      cliCfg.workspace_id = existingCliCfg.workspace_id;
    }
    await mkdir(cliProfileDir(profile), { recursive: true });
    await writeFile(
      cliProfileConfigPath(profile),
      JSON.stringify(cliCfg, null, 2),
      "utf-8",
    );
  };
...
async function savePrefs(prefs: DaemonPrefs): Promise<void> {
  const dir = join(homedir(), ".multicaa");
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  ...
  if (
    process.env.DESKTOP_APP_NAME === "Simultica" &&
    !env.MULTICA_TRAEX_PATH
  ) {
    env.MULTICA_TRAEX_PATH = "/Users/bytedance/.local/bin/traex";
  }
  return env;
}
