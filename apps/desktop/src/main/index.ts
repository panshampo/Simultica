...
const PROTOCOL = process.env.DESKTOP_PROTOCOL || "multica";
const SHOULD_REGISTER_PROTOCOL =
  process.env.DESKTOP_REGISTER_PROTOCOL !== "0";
...
const DEV_APP_NAME = process.env.DESKTOP_APP_NAME
  ? process.env.DESKTOP_APP_NAME
...
if (is.dev) {
  ...
  app.setName("Multicaa");
}

// --- Protocol registration -----------------------------------------------

if (SHOULD_REGISTER_PROTOCOL) {
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev
        ? process.env.DESKTOP_APP_USER_MODEL_ID || "ai.multica.desktop.dev"
        : "ai.multica.desktop",
    );
    ...
    ipcMain.handle(
      "runtime-config:save",
      (_event, input: RuntimeConfigInput) => saveRuntimeConfig(input),
    );
