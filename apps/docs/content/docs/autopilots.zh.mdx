---
title: 自动化
description: 让智能体从定时、webhook、手动触发、内联 issue 配置或 Issue Template 自动开工。
---

import { Callout } from "fumadocs-ui/components/callout";

自动化让 [智能体](/agents) **自动开工**——可以来自定时调度、入站 webhook，也可以手动触发一次。适合定期巡检、周期性报告、凌晨跑的清理任务这类"standing order"场景。和前三种触发方式（[分配](/assigning-issues) / [@ 提及](/mentioning-agents) / [对话](/chat) 都是你主动喊一声）相比，自动化由一条保存好的触发配置驱动。

<Callout type="info">
部分旧 UI、路由和 CLI 仍然使用 Autopilot 这个词。产品语义上，这一页讲的是自动化。自动化可以用内联 issue 配置或 [Issue Template](/issue-templates) 创建 Issue，也可以在直跑模式下直接入队 task。它不会创建 Workflow，也不会创建 Workflow 产品里的 Run。
</Callout>

## 配置一条自动化

在工作区的 **Automations** 页新建一条自动化，要定下：
...
- **直跑模式**（`run_only`）—— 不建 issue，直接入队一个 `task`。看板上看不到这一次运行——只能在自动化运行历史里看到。

## 选择 issue 来源

先建 issue 模式有两种来源：

- **内联 issue 配置** —— automation 自己保存 issue 标题、正文、负责人、优先级和 project 设置。
- **Issue Template** —— automation 引用一个已有 Issue Template，每次触发时从这个模板创建新 Issue。

如果同一套 Issue 形态既要手动创建，也要自动创建，就用 Issue Template。如果这条自动化是一次性的、不需要复用模板，就用内联 issue 配置。

<Callout type="warning">
Issue Template 只创建 Issue。不要用 Issue Template 表达 Workflow、Skill 或 Agent 配置。
</Callout>

## 让它按时间跑

每条定时自动化至少要一个 `schedule` 触发器。Cron 是**标准 5 字段格式**（分 时 日 月 周），最小粒度 **1 分钟**（没有秒级）。时区用 IANA 格式（例如 `Asia/Shanghai`），决定 cron 表达式按哪个时区解读。
...
Si-Multica 服务器每 **30 秒**扫一次到期的触发器——**触发时刻最多延迟 30 秒**，不是秒级精准。服务器重启时如果恰好错过触发点，启动时会补扫漏掉的触发（不会丢触发，但会立刻补跑）。

## 手动触发一次

调试自动化时不想等 cron，可以手动触发一次：
...
手动触发走和 `schedule` 触发完全相同的执行流程，只是自动化运行记录里 `source` 字段标为 `manual`。

## 通过 Webhook 触发

自动化也可以由入站 HTTP webhook 触发。在详情页添加一个 **Webhook**
触发器，Si-Multica 会生成一个唯一的 URL：

```
https://<你的 Si-Multica host>/api/webhooks/autopilots/awt_…
```

向这个 URL POST 任意 JSON——Si-Multica 会记录一条 `source = webhook` 的
...
也可以直接发任意 JSON 对象 / 数组。Si-Multica 会规范化为内部封装：
...
不带 `event` 字段时，Si-Multica 会按以下顺序从常见 header 和 body 字段
...
Si-Multica 按下面的顺序从入站请求里推断 `event` 和 `action`，**先命中先用**。
...
生成的 URL **就是凭证**，谁拿到都能触发这条自动化。请按 token 对待：
...
- 对需要强来源认证的源，等 per-trigger HMAC 签名校验上线；v1 URL 仅
  bearer。
- 当前能查看这条自动化的工作区成员都能看到它的 webhook URL——更细的
  权限可见性是后续工作。
...
- `{"status":"ignored","reason":"autopilot_paused"}` —— 自动化已暂停。
- `{"status":"ignored","reason":"autopilot_archived"}` —— 自动化已归档。
...
但自定义反向代理就不行了。Si-Multica **故意不**从 `Host` /
`X-Forwarded-Host` header 推断公开主机，避免反代配置失误时被诱导生成
指向攻击者域名的 webhook URL。

## 看运行历史

每次触发都会产生一条**自动化运行记录**（run），可以在自动化详情页的"历史"tab 看到：
...
<Callout type="warning">
**自动化失败不自动重试，也不发 inbox 通知。** 失败后只在运行历史里留一条 `failed` 记录——不会像分配 / @ 提及那样由系统重新排队，也不会给任何人发通知。如果这条自动化是周期任务，**下一次 cron 到点会重新触发一次**（新的 run），但这一次失败的工作不会被自动补跑。

如果自动化很重要，要自己设计监控——例如让智能体在成功时给自己发个评论，通过缺失评论来发现失败。
</Callout>

不自动重试的理由：自动化通常本身是周期性的，系统层再加自动重试容易和下一次调度叠加，产生重复执行。调度权完全交给 cron 最干净。
...
- [**Issue Templates**](/issue-templates) —— 用于手动创建和自动化创建 issue 的可复用形态
- [**Workflow 和 Issue 如何协作**](/workflow-issues) —— 复杂或高风险 AI 工作的分阶段运行
