...
## Issue workflow 里的 Skill

当智能体为一条 issue 规划工作时，Skill 会变得尤其重要。Skill 仍然是可复用能力包；智能体可以使用其中的指令或 workflow template，为当前 issue 生成 runtime workflow。

完整模型见 [Workflow 和 Issue 如何协作](/workflow-issues)：这页解释 skill、主 issue、runtime workflow、子 issue 和 `workflow_run` 如何配合。
