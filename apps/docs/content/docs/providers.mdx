...
From ByteDance GEC. DS Copilot is a Claude Code wrapper with a managed CCR model proxy and data-science tooling. It uses Claude Code's stream-json protocol, session resume, and MCP flags, but **model selection is managed by the DS Copilot launcher** (`.last_model` / CCR config). Multica therefore disables per-agent model selection for this provider and does not append `--model`.
...
From ByteDance. Traex is a Codex fork that exposes the same `app-server --listen stdio://` JSON-RPC entry point as Codex. Multica reuses the Codex transport and model catalog. MCP config is materialized into the per-task Traex home, and skills come from `$TRAE_HOME/skills` (default `~/.trae/skills`).
