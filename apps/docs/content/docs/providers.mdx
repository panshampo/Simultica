...
description: Si-Multica supports 14 AI coding tools; they implement the same interface, but the capability details diverge significantly.
---

import { Callout } from "fumadocs-ui/components/callout";

Si-Multica ships with built-in support for **14 AI coding tools**. They all implement the same interface — queue, dispatch, execute, return results — so you can drive any of them from the same Si-Multica board. **But the capability details diverge significantly**: whether session resumption actually works, whether MCP is supported, where skill files live, how models are selected. This page is the full matrix.
...
| **DS Copilot** | ByteDance GEC | ✅ | ✅ | `.ds-copilot/skills/` | Managed by DS Copilot launcher/CCR |
...
| **Traex** | ByteDance | ⚠️ Code exists but unreachable | ✅ | `$TRAE_HOME/skills/` | Static |
...
From ByteDance GEC. DS Copilot is a Claude Code wrapper with a managed CCR model proxy and data-science tooling. It uses Claude Code's stream-json protocol, session resume, and MCP flags, but **model selection is managed by the DS Copilot launcher** (`.last_model` / CCR config). Si-Multica therefore disables per-agent model selection for this provider and does not append `--model`.
...
From ByteDance. Traex is a Codex fork that exposes the same `app-server --listen stdio://` JSON-RPC entry point as Codex. Si-Multica reuses the Codex transport and model catalog. MCP config is materialized into the per-task Traex home, and skills come from `$TRAE_HOME/skills` (default `~/.trae/skills`).
...
|---|---|---|
| ✅ Really works | Antigravity, Claude Code, DS Copilot, Copilot, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Pi | Pass the resume id and it continues from the previous context |
| ⚠️ Code exists but unreachable | Codex, Cursor, Traex | Resume paths exist in the code but aren't actually reached (Codex/Traex silently fall back; Cursor doesn't return session id) — **treat as unsupported** |
...
**Of the 14 tools, nine consume `mcp_config`: Claude Code, DS Copilot, Codex, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, and Traex**. The other five accept the field but **ignore it** — no error, no warning, the config just has no effect.

The runtime paths are provider-specific: Claude Code and DS Copilot receive it through `--mcp-config` paired with `--strict-mcp-config`; Codex and Traex write a daemon-managed `mcp_servers` block into the per-task home config; Hermes, Kimi, and Kiro CLI receive ACP `mcpServers`; OpenCode receives inline config through `OPENCODE_CONFIG_CONTENT`; OpenClaw receives `mcp.servers` through Si-Multica's per-task config wrapper. OpenCode's path does **not** rewrite the project's `opencode.json`.
...
Each tool uses **its own** skill discovery path. Before a task runs, the Si-Multica daemon copies the workspace's skill files into the corresponding path:

| Tool | Path | Native discovery? |
|---|---|---|
| Claude Code | `.claude/skills/` | ✅ Native |
| DS Copilot | `.ds-copilot/skills/` | ✅ Native |
...
- [Daemon and runtimes](/daemon-runtimes) — where the tools run and how they connect to Si-Multica
