...
| インストール | 社内の DS Copilot インストーラーに従ってください。バイナリが `PATH` にない場合、Multica はデフォルトの launcher script を使用できます。必要に応じて `MULTICA_DS_COPILOT_PATH` に実行可能 wrapper または `~/.ds-copilot/launch.sh` を設定してください。 |
| 認証 | `ds-copilot --doctor` を実行し、credentials / CCR / data-service の問題を解消してから Multica のタスクに使ってください。 |
| 備考 | Multica は DS Copilot に `--model` を渡しません。モデル変更は `ds-copilot --pick-model` または launcher の保存済みモデルで行ってください。 |
