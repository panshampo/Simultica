...
description: Si-Multica はあなたのマシンにインストールされている AI コーディングツールを駆動します。このページでは、デーモンがそれらを検出できるように、サポートされている 14 種のツールをそれぞれインストールする方法を説明します。
---

import { Callout } from "fumadocs-ui/components/callout";

Si-Multica における**ランタイム**とは、あなたのマシンのデーモンと、デーモンが `PATH` で見つけた AI コーディングツール 1 つが組になったものです。オンボーディングの「ランタイムを接続」ステップで **No supported tools detected** と表示される場合、それはデーモンが `PATH` をスキャンしたものの、駆動方法を知っている 14 種のツールのいずれも見つけられなかったことを意味します。以下のツールのいずれか（または複数）をインストールしてから、そのステップに戻って再スキャンしてください — 数秒以内にランタイムが表示されます。
...
<Callout type="info">
Si-Multica サーバーがあなたの API キーやツール自体を見ることは決してありません。以下のすべて — インストール、認証、モデルアクセス — はあなたのローカルマシン上に存在します。何かが失敗する場合、それはほぼ常にローカルの問題です。
</Callout>
...
1. **Si-Multica デーモンが実行中である必要があります。** [Si-Multica CLI](/cli) をインストールした後に `multica daemon start` を実行するか、デーモンを自動的に起動する [Si-Multica デスクトップアプリ](/desktop-app)を使用してください。デーモンが実行されていなければ、ツールを検出する主体がありません。
...
ByteDance のデータサイエンス向け Claude Code wrapper です。Claude Code の stream-json プロトコルを使いますが、モデル選択は DS Copilot launcher と CCR config が管理します。

| | |
|---|---|
| デーモンが探す名前 | `ds-copilot`、次に `~/.ds-copilot/launch.sh` |
| インストール | 社内の DS Copilot インストーラーに従ってください。バイナリが `PATH` にない場合、Si-Multica はデフォルトの launcher script を使用できます。必要に応じて `MULTICA_DS_COPILOT_PATH` に実行可能 wrapper または `~/.ds-copilot/launch.sh` を設定してください。 |
| 認証 | `ds-copilot --doctor` を実行し、credentials / CCR / data-service の問題を解消してから Si-Multica のタスクに使ってください。 |
| 備考 | Si-Multica は DS Copilot に `--model` を渡しません。モデル変更は `ds-copilot --pick-model` または launcher の保存済みモデルで行ってください。 |
...
Codex と同じ `app-server --listen stdio://` JSON-RPC transport を公開する Codex fork です。

| | |
|---|---|
| デーモンが探す名前 | `traex` |
| インストール | Traex のインストーラーに従ってください。必要であれば `MULTICA_TRAEX_PATH` に実行可能ファイルを設定してください。 |
| 認証 | `traex --version` と小さなローカル prompt を一度実行し、CLI が使える状態か確認してください。 |
...
オープンソースの CLI エージェントオーケストレーターです。**モデルはエージェント層にバインドされます**（`openclaw agents add --model`） — タスクごとに上書きすることはできず、Si-Multica から `--model` や `--system-prompt` を渡すこともできません。
...
| 備考 | CLI は構造化されたイベントストリームではなく、stdout に通常のアシスタントテキストを出力します。途中の「I will run X」の行と最終的な応答の両方がテキストとして Si-Multica に中継されます。 |
...
3. **ランタイムページを確認してください。** Si-Multica UI の**ランタイム**ページに、`(ワークスペース × ツール)` の組み合わせごとに 1 行ずつ表示されるはずです。行に「offline」と表示される場合は、[デーモンとランタイム → ランタイムがオフラインと表示されるとき](/daemon-runtimes#when-a-runtime-is-marked-offline)を参照してください。
