...
description: Bind a Si-Multica agent to a Lark (飞书) Bot, then talk to it from a Lark DM or group — @-mention it, chat naturally, or type /issue to file a Si-Multica issue without leaving Lark.
---

import { Callout } from "fumadocs-ui/components/callout";

Bind any [agent](/agents) to a Lark (飞书) Bot and your team can work with it from inside Lark — DM the Bot, @-mention it in a group, or type `/issue` to file a [Si-Multica issue](/issues) without opening the app. The agent's replies stream back into the chat as a live card that updates while it works.

Each Bot is bound **one-to-one** to a single Si-Multica agent. Binding a second agent creates a second Bot; one agent never has two Bots.
...
| **DM the Bot** | A workspace member messages the Bot directly in Lark. Each conversation becomes a Si-Multica [chat](/chat) session with the agent; the agent answers in-thread. |
...
| **`/issue` command** | Typing `/issue <title>` (optionally with a body) creates a new Si-Multica issue in the workspace, attributed to you. |
...
4. When the scan completes the dialog closes and the agent shows **Connected to Lark**. Your own Lark identity is bound to your Si-Multica account automatically, so you can start chatting with the Bot right away.
...
The first time you message the Bot, it replies with a card asking you to **bind your Lark identity**. Tap the link, sign in to Si-Multica, and your Lark account is linked to your Si-Multica membership. This is what lets the agent act as you — for example, `/issue` files the issue under your name.
...
- **File an issue** — send `/issue Fix the login redirect` and Si-Multica creates that issue in the workspace, assigned the way any new issue would be. Add more lines after the title for a description.
...
The agent can embed images (charts, diagrams, screenshots) in its final reply. It writes a plain markdown link like `![chart](./out/cpu.png)` in the answer; the runtime uploads the matching local file to Lark, and the card renders the image inline.

Limits:

- **PNG and JPG only**; each image must be **≤ 10 MiB** — oversized files are silently dropped.
- Paths must stay inside the task workdir — absolute paths and `..` traversal are rejected.
- Only images actually referenced in the final reply are uploaded; other files on disk are ignored.
- `http(s)://` remote image links pass through unchanged — the runtime does not download them.
- **At most 5 images per reply** — additional images are truncated, with a notice on the card.
- Upload is best-effort: if every image fails, the reply degrades to plain text/Markdown and the task does not fail.
...
On Si-Multica Cloud the integration is already available — skip this section.
...
- [Chat](/chat) — what a Bot conversation maps to inside Si-Multica
