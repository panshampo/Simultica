...
description: Common issues when self-hosting Si-Multica — symptoms, causes, how to diagnose, how to fix.
---
...
| `smtp RCPT TO <addr>: 550 5.7.1 Unable to relay` | The relay's receive connector doesn't allow your subnet to relay to external recipients (most common for anonymous internal relays talking to outside domains) | Either restrict invites to internal recipients, or add the Si-Multica host's subnet to the Exchange "Anonymous Users → Relay" permission list |
...
- `Unable to relay` → either restrict to internal recipients or grant the Si-Multica host's IP relay permission on the Exchange receive connector
...
2. **`pg_cron` is configured for compatibility but pointing at the wrong database** — `pg_cron.database_name` defaults to `postgres`; if your Si-Multica database has a different name, the scheduled job never sees `rollup_task_usage_hourly()`. The in-process scheduler does not depend on this, but if you removed the in-process scheduler and rely on `pg_cron`, the DB name must match.
