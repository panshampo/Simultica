---
title: Automations
description: Start agent work from schedules, webhooks, manual triggers, inline issue config, or Issue Templates.
---

import { Callout } from "fumadocs-ui/components/callout";

Automations let [agents](/agents) **start work automatically** — from a schedule, an inbound webhook, or a manual trigger. They fit periodic checks, recurring reports, and overnight cleanup jobs: the "standing order" shape of work. Compared to the other three trigger paths ([assigning](/assigning-issues), [@-mention](/mentioning-agents), and [chat](/chat), where you are the one kicking things off), Automations are owned by a saved trigger configuration.

<Callout type="info">
Some older UI labels, routes, and CLI commands still use the word "Autopilot." Product-wise, this page describes Automations. They either create an Issue from inline issue settings or from an [Issue Template](/issue-templates), or they enqueue a task directly in run-only mode. They do not create Workflows or Runs for the Workflow product.
</Callout>

## Configure an Automation

Create a new automation on the workspace's **Automations** page. You set:
...
An automation has two execution modes. **Start with "create issue" mode.**
...
- **Run-only mode** (`run_only`) — skips issue creation and enqueues a `task` directly. The run is invisible on the board — you can only see it in the automation's run history.

## Pick an issue source

Create issue mode has two source shapes:

- **Inline issue config** — the automation stores its own issue title, body, assignee, priority, and project settings.
- **Issue Template** — the automation references an existing Issue Template and creates each new Issue from that template.

Use an Issue Template when the same issue shape is useful both manually and automatically. Use inline issue config when the automation is one-off and does not need a reusable template.

<Callout type="warning">
Issue Templates only create Issues. Do not use an Issue Template to represent a Workflow, Skill, or Agent setup.
</Callout>

## Run it on a schedule

Every scheduled automation needs at least one `schedule` trigger. Cron uses the **standard 5-field format** (minute hour day month weekday), with **1-minute** minimum granularity (no seconds). Timezone is IANA-formatted (for example, `Asia/Shanghai`) and determines which timezone the cron expression is interpreted in.
...
The Si-Multica server scans for due triggers every **30 seconds** — **the actual fire time can lag by up to 30 seconds**, not down to the second. If the server is restarted across a fire time, it catches up missed triggers on startup (nothing is lost, but they fire right away).

## Trigger once manually

To avoid waiting for cron while debugging an automation, trigger it manually:

- UI: click "Run now" on the automation detail page
- CLI:
...
A manual trigger goes through the exact same execution flow as a `schedule` trigger — only the `source` field on the automation run record is marked `manual`.

## Trigger from a webhook

Automations can also fire on inbound HTTP webhooks. Add a **Webhook** trigger
on the automation detail page; Si-Multica generates a unique URL of the shape:

```
https://<your-multica-host>/api/webhooks/autopilots/awt_…
```

POST any JSON to that URL — Si-Multica records a run with `source = webhook`,
...
…or any JSON object/array. Si-Multica normalizes it into an internal envelope:
...
When you don't provide an `event` field, Si-Multica infers it from common
...
comma-separated **actions** list. Si-Multica allows a webhook if **any**
...
Si-Multica derives the `event` name and `action` from the inbound request
...
**2. Headers.** When no body envelope is present, Si-Multica reads the
...
present, Si-Multica falls back to top-level body string fields in this
...
**Action candidates, in full.** Once the event is determined, Si-Multica
...
automation. Treat it like a token:
...
- Workspace members who can view the automation can read its webhook URLs
  for now — tighter per-role secret visibility is a follow-up.

### Status-code semantics

Si-Multica returns `200 OK` with a `status` field for normal no-op outcomes so
...
- `{"status":"ignored","reason":"autopilot_paused"}` — the automation is paused.
- `{"status":"ignored","reason":"autopilot_archived"}` — the automation is archived.
...
custom self-hosted reverse proxies. Si-Multica deliberately does not derive
...
Every trigger produces an **automation run record**, visible on the "History" tab of the automation detail page:
...
<Callout type="warning">
**Automation failures are not auto-retried and do not send inbox notifications.** A failure leaves a `failed` entry in run history — no system-level re-enqueue like assign or @-mention, and no notification to anyone. If the automation is periodic, **the next cron fire will trigger a new run**, but the failed work is not automatically re-run.

If an automation is important, design your own monitoring — for example, have the agent post a comment on success, and catch failures by noticing missing comments.
</Callout>

Why no auto-retry: automations are often already periodic, so adding system-level retries stacks on top of the next scheduled run and creates duplicate executions. Leaving the schedule entirely to cron keeps it clean.
...
- [**Issue Templates**](/issue-templates) — reusable issue shapes for manual issue creation and automation
- [**Workflow and issues**](/workflow-issues) — staged execution for complex or risky AI work
