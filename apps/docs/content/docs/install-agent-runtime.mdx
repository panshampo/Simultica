...
| Install | Follow your internal DS Copilot installer. If the binary is not on `PATH`, Multica can use the default launcher script. You can also set `MULTICA_DS_COPILOT_PATH` to an executable wrapper or to `~/.ds-copilot/launch.sh`. |
| Authentication | Run `ds-copilot --doctor` and fix any local credentials / CCR / data-service failures before assigning Multica tasks to it. |
| Notes | Multica does not pass `--model` to DS Copilot; use `ds-copilot --pick-model` or the launcher's saved model file to change it. |
