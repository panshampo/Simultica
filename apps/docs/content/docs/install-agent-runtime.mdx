...
description: Si-Multica drives whichever AI coding tools you have on your machine. This page shows you how to install each of the 14 supported tools so the daemon can detect them.
---

import { Callout } from "fumadocs-ui/components/callout";

A **runtime** in Si-Multica is the daemon on your machine paired with one AI coding tool the daemon found on your `PATH`. If the onboarding "Connect a runtime" step shows **No supported tools detected**, it means the daemon scanned `PATH` and didn't find any of the 14 tools it knows how to drive. Install one (or several) of the tools below, then come back to the step and re-scan — the runtime will show up within a few seconds.
...
<Callout type="info">
The Si-Multica server never sees your API keys or the tools themselves. Everything below — installation, authentication, model access — lives on your local machine. If something fails, it's almost always a local problem.
</Callout>
...
1. **The Si-Multica daemon must be running.** Either run `multica daemon start` after installing the [Si-Multica CLI](/cli), or use the [Si-Multica desktop app](/desktop-app), which launches the daemon automatically. Without a running daemon there is nothing to detect tools.
...
### DS Copilot (ByteDance GEC)

Claude Code wrapper for ByteDance data-science workflows. It uses Claude Code's stream-json protocol, but model selection is managed by the DS Copilot launcher and CCR config.

| | |
|---|---|
| Daemon looks for | `ds-copilot`, then `~/.ds-copilot/launch.sh` |
| Install | Follow your internal DS Copilot installer. If the binary is not on `PATH`, Si-Multica can use the default launcher script. You can also set `MULTICA_DS_COPILOT_PATH` to an executable wrapper or to `~/.ds-copilot/launch.sh`. |
| Authentication | Run `ds-copilot --doctor` and fix any local credentials / CCR / data-service failures before assigning Si-Multica tasks to it. |
| Notes | Si-Multica does not pass `--model` to DS Copilot; use `ds-copilot --pick-model` or the launcher's saved model file to change it. |
...
Codex fork that exposes the same `app-server --listen stdio://` JSON-RPC transport.

| | |
|---|---|
| Daemon looks for | `traex` |
| Install | Follow your Traex installer. If needed, set `MULTICA_TRAEX_PATH` to the executable. |
| Authentication | Run `traex --version` and a small local prompt once to verify the CLI is ready. |
...
Open-source CLI agent orchestrator. MCP config is materialized through Si-Multica's per-task config wrapper. **Model is bound at the agent layer** (`openclaw agents add --model`) — it can't be overridden per task, and you can't pass `--model` or `--system-prompt` from Si-Multica.
...
Google's Antigravity CLI (`agy`). Pairs with Google's Antigravity service and runs Gemini-backed models. Session resumption works through `--conversation <id>`, captured by the daemon from the CLI log file. Model selection is managed inside the Antigravity CLI itself — Si-Multica disables the per-agent model picker for this provider. Skills are written to `.agents/skills/` (the CLI inherits Gemini CLI's workspace skill layout — see [Antigravity docs](https://antigravity.google/docs/gcli-migration)).
...
| Notes | The CLI emits plain assistant text on stdout, not a structured event stream; intermediate "I will run X" lines and the final reply are both relayed to Si-Multica as text. |
...
3. **Check the Runtimes page.** In the Si-Multica UI, the **Runtimes** page should now list one row per `(workspace × tool)` combination. If the row says "offline", see [Daemon and runtimes → When a runtime is marked offline](/daemon-runtimes#when-a-runtime-is-marked-offline).
