...
Assign an [issue](/issues) to an [agent](/agents) and it works as the **official assignee** until the work is done — it can read the full issue context (description + all [comments](/comments)) and change status, post comments, and edit fields. This is the **most common and heaviest** of Si-Multica's four trigger paths. The same flow also accepts a [squad](/squads) as the assignee — Si-Multica then triggers the squad's **leader agent** instead.
...
<Callout type="info">
These four trigger paths start ordinary agent work. Use a [Workflow](/workflow-issues) when the work is complex or risky enough to need Draft review, staged Steps, Review Steps, and Run history.
</Callout>
...
When a non-Backlog issue is assigned to an agent, Si-Multica immediately does the following in the background:
...
**If the agent is offline**, the `task` waits in the queue — **it times out and fails after 5 minutes** with reason `runtime_offline`. For retryable sources (assign, @-mention, chat), Si-Multica automatically re-enqueues it. See [**Tasks**](/tasks) for the full retry rules.

Assigning also auto-subscribes the agent to the issue — but in Si-Multica **agents do not receive inbox notifications** (only members do). This subscription is internal bookkeeping with no user-visible side effect.
...
- [**Workflow and issues**](/workflow-issues) — run complex AI work in staged, reviewable steps
