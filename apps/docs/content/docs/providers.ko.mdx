...
ByteDance GEC에서 제공합니다. DS Copilot은 managed CCR 모델 프록시와 데이터 사이언스 도구를 포함한 Claude Code wrapper입니다. Claude Code의 stream-json 프로토콜, 세션 재개, MCP flag를 사용하지만, **모델 선택은 DS Copilot launcher**(`.last_model` / CCR config)가 관리합니다. 따라서 Multica는 이 provider에서 에이전트별 모델 선택을 비활성화하고 `--model`을 추가로 전달하지 않습니다.
...
ByteDance에서 제공합니다. Traex는 Codex와 같은 `app-server --listen stdio://` JSON-RPC entry point를 노출하는 Codex fork입니다. Multica는 Codex transport와 모델 카탈로그를 재사용합니다. MCP 구성은 작업별 Traex home에 기록되며, 스킬은 `$TRAE_HOME/skills`(기본값 `~/.trae/skills`)에서 읽습니다.
