...
- **From GitHub** — paste a repo URL (e.g. `https://github.com/owner/repo/tree/main/skills/my-skill`) and Si-Multica pulls the `SKILL.md` and every file in that directory
...
## Skills inside Workflows

Skills become especially useful when an agent drafts a Workflow. The skill stays a reusable capability pack, while the agent may use its instructions or workflow shapes to draft the right steps for a specific goal.

For the full model, see [Workflow and issues](/workflow-issues): it explains how Skills, Issues, Workflows, Runs, Steps, and review steps fit together.

## Safety of third-party skills

Skills imported from GitHub or ClawHub may include scripts and executable content. Si-Multica itself **does not sign, audit, or sandbox them** — skill contents are handed to the corresponding AI coding tool as-is, and whether the tool treats them as executable is up to the tool.
...
The two are complementary. In Si-Multica today, MCP support is **provider-specific**: Claude Code, DS Copilot, Codex, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, and Traex consume `mcp_config`; other tools receive the field but don't actually use it. A dedicated MCP section will come in a later release.
