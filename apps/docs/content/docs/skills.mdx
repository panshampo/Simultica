...
## Skills inside issue workflows

Skills become especially useful when an agent plans work for an issue. The skill stays a reusable capability pack, while the agent may use its instructions or workflow templates to build a runtime workflow for the specific issue.

For the full model, see [Workflow and issues](/workflow-issues): it explains how skills, main issues, runtime workflows, sub-issues, and `workflow_run` fit together.
...
The two are complementary. In Multica today, MCP support is **provider-specific**: Claude Code, DS Copilot, Codex, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, and Traex consume `mcp_config`; other tools receive the field but don't actually use it. A dedicated MCP section will come in a later release.
