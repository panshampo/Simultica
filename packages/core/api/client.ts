...
export class ApiClient {
  ...
  async listSkillFiles(skillId: string): Promise<SkillFile[]> {
    return this.fetch(`/api/skills/${skillId}/files`);
  }

  async upsertSkillFile(skillId: string, data: { path: string; content: string }): Promise<SkillFile> {
    return this.fetch(`/api/skills/${skillId}/files`, {
      method: "PUT",
      body: JSON.stringify(data),
    });
  ...
  async deleteSkillFile(skillId: string, fileId: string): Promise<void> {
    await this.fetch(`/api/skills/${skillId}/files/${fileId}`, { method: "DELETE" });
  }
