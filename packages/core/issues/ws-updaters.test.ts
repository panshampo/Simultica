...
describe("project progress invalidation", () => {
  ...
  it("hydrates the issue detail cache on issue create", () => {
    const created = {
      ...baseIssue,
      project_id: PROJECT_ID,
      title: "Created live",
    ...
    onIssueCreated(qc, WS_ID, created);

    expect(qc.getQueryData<Issue>(issueKeys.detail(WS_ID, ISSUE_ID))).toEqual(created);
  });
});
