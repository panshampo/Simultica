...
export interface Agent {
  ...
  /**
   * Project used by the Lark bot integration for per-thread auto-created
   * issues. `null` means group-chat auto-issue creation is disabled for
   * this agent; p2p chat remains unchanged. Older backends omit the field.
   */
  lark_issue_project_id?: string | null;
  owner_id: string | null;
...
export interface CreateAgentRequest {
  ...
  template?: string;
  /** Optional project where Lark group-chat @-mention threads auto-create issues. */
  lark_issue_project_id?: string;
}
...
export interface UpdateAgentRequest {
  ...
  thinking_level?: string;
  /**
   * Lark auto-issue project. Tri-state semantics:
   *   - field omitted -> no change
   *   - "" -> clear / disable auto-issue creation
   *   - UUID -> set the project used for group-chat thread issues
   */
  lark_issue_project_id?: string;
}
