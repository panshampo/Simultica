      ...
      <section className="rounded-lg border">
        ...
        <div className="border-t px-4 py-3">
          {!configured ? (
            ...
            // bot: project routing is optional and independent from the
            // shared bind entry, which renders scan-to-bind or connected.
            <div className="space-y-4">
              <div className="flex flex-col gap-3 rounded-md border bg-muted/20 p-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0 space-y-1">
                  <p className="text-xs font-medium">
                    {t(($) => $.tab_body.integrations.auto_issue_title)}
                  </p>
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    {t(($) => $.tab_body.integrations.auto_issue_description)}
                  </p>
                </div>
                <div className="shrink-0">
                  <ProjectPicker
                    projectId={agent.lark_issue_project_id || null}
                    align="end"
                    triggerClassName="h-6 max-w-40 rounded-md px-1.5 text-xs font-medium text-foreground"
                    onUpdate={(updates) => {
                      void onUpdate(agent.id, {
                        lark_issue_project_id: updates.project_id ?? "",
                      });
                    }}
                  />
                </div>
