...
export function ActivityTab({ agent }: ActivityTabProps) {
  ...
  const isIssueBackedChatTask = (t: AgentTask) => !!t.chat_session_id && t.issue_id !== "";
  // Private chat tasks stay in the chat UI. Lark issue-backed chat tasks
  // also carry chat_session_id, but they are team-visible workflow runs and
  // need an Agent Activity entry while active.
  const isWorkflowTask = (t: AgentTask) => !t.chat_session_id || isIssueBackedChatTask(t);
