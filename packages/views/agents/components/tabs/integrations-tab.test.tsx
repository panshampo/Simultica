...
vi.mock("../../../projects/components/project-picker", () => ({
  ProjectPicker: ({
    ...
    <div
      data-testid="auto-issue-project-picker"
    ...
    >
      ...
      <button type="button" onClick={() => onUpdate({ project_id: null })}>
        clear project
      </button>
    </div>
  ),
...
function renderIntegrationsTab(
  ...
  renderTab(<IntegrationsTab agent={nextAgent} onUpdate={onUpdate} />);
  return { onUpdate };
}
...
describe("IntegrationsTab", () => {
  ...
  it("keeps the auto-issue project optional and maps selection to the agent field", () => {
    const onUpdate = vi.fn().mockResolvedValue(undefined);
    renderIntegrationsTab(
      { ...agent, lark_issue_project_id: null },
      onUpdate,
    );

    expect(screen.getByTestId("lark-bind-button")).toBeTruthy();
    const picker = screen.getByTestId("auto-issue-project-picker");
    expect(picker).toBeTruthy();
    expect(picker.getAttribute("data-trigger-class")).toContain("h-6");
    expect(picker.getAttribute("data-trigger-class")).toContain("text-xs");

    fireEvent.click(screen.getByRole("button", { name: /pick project/i }));
    expect(onUpdate).toHaveBeenCalledWith("agent-1", {
      lark_issue_project_id: "project-1",
    });
