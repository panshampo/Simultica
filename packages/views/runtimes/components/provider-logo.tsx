...
// TraeX (ByteDance) — Codex-fork CLI; rendered as a rounded mono-square
// "T" mark in TraeX brand red so it reads as its own runtime rather
// than an OpenAI variant in dense provider lists.
function TraexLogo({ className }: { className: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className}>
      <rect width="24" height="24" rx="5" fill="#E63946" />
      <path
        d="M5.5 6.5h13v2.6h-5.1V18h-2.8V9.1H5.5V6.5z"
        fill="#FFFFFF"
      />
    </svg>
  );
...
function DsCopilotLogo({ className }: { className: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className}>
      <rect width="24" height="24" rx="5" fill="#111827" />
      <path
        d="M5.5 6.25h6.15c4.15 0 6.85 2.35 6.85 5.75s-2.7 5.75-6.85 5.75H5.5V6.25zm2.85 2.35v6.8h3.12c2.45 0 4.05-1.33 4.05-3.4s-1.6-3.4-4.05-3.4H8.35z"
        fill="#FFFFFF"
      />
      <path
        d="M16.95 6.25h1.55v11.5h-1.55l-1.5-1.95V8.2l1.5-1.95z"
        fill="#7DD3FC"
      />
    </svg>
  );
...
export function ProviderLogo({
  ...
  switch (provider) {
    ...
    case "ds-copilot":
      return <DsCopilotLogo className={className} />;
    case "codex":
    ...
    case "traex":
      return <TraexLogo className={className} />;
    default:
